...
- **Bulk onboarding, assignment and moves.** Unknown identities become devices in one go, a selection of devices joins a project from its first data with an entity each if wanted, a selection of devices in a project gets an entity each with a name per row, and a device or an entity moves to another project with its history from a chosen moment, after a preview of what comes along.
...
- **Cardiac monitoring.** A LINQII tag the animal wears is heard by its device and its heart rate, variability, activity and body temperature are stored as measurements; an analysis reads them by day, night and resting, as a rhythm, over the period and before and after an event, with what the device heard first.
