...
**Status: v2.8.0 released on 2026-09-19**, with contact tracing end to end: Bluetooth sightings as a canonical record type of their own, a device's place set by hand, and the analysis module that reads sightings beside position proximity with the warnings that keep the figures honest. The same tag carries the Sentinel-2 vegetation of each management area in the grazing analysis, a Dutch interface, the AWT tracker driver, a battery type per device and a gateway named by hand; see `CHANGELOG.md`. `v2.7.0` was skipped, as phase 29's movement half is still to build. The documentation site is at https://smartparksorg.github.io/smartparks-protect/.
