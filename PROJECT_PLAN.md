...
| Field | Value |
| --- | --- |
| Active phase | Phase 14 complete except the v2.0.0 release (everything committed through ad70957 plus the nginx documentation fix, CI green, dev server on it). Live verification with Tim's ChirpStack (chirpstack-dev4) proven in both directions on 2026-09-05: uplinks of collar SP051307 over the HTTP integration, Test connection and Sync devices over gRPC, and a Request status downlink queued in ChirpStack |
| Latest release | v0.6.0 (2026-09-04): phases 7, 8 and 9; phases 10 to 13 and the deployment fixes unreleased |
| Last session | 2026-09-05 |
| Next item | Confirm the round trip: the collar's next uplink collects the queued Request status, ChirpStack's acknowledgement reaches the webhook (command becomes sent), the status answer decodes; then Sync gateways, and the four other DevEUIs waiting in Needs attention (three OpenCollars, one A81758... Elsys-like sensor) get their devices or stay ignored. Then v2.0.0 on Tim's word (a docs-only deployment lands on v0.6.0 and fails the security check until 2.0 is tagged); decide whether a fresh install seeds the OpenCollar device type; the ChatGPT half of the phase 9 check waits for a Pro or Business account; the live items of phases 7, 8, 11 and 13 follow the accounts, a collar with BLE, a The Things Stack application, a ThingPark deployment and an EarthRanger site |
