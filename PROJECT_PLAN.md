...
| Next item | Phase 15 in order: run the SP051307 repair on the dev server after the deploy, then device health and last seen (D104), traffic columns per channel, gateway metadata, the firmware-aware driver (D100), the metrics review (D102); then getting around, organising, presentation, coverage, and v2.0.0. Also open: Request status through KPN for the complete timeline, Sync gateways and the four DevEUIs on ChirpStack, the live items that wait for other accounts |
