...
- [ ] Webhook: `webhook_token_in_query` on the ChirpStack adapter (D127), so `?token=` authenticates as the Authorization header does; the token in the query never reaches the audit or the traces. Test in `tests/api/test_ingest_and_attention.py` or a new `test_chirpstack_connect.py`.
- [ ] The encrypted copy (D125): `webhook_token` kept in `credentials_encrypted` of a ChirpStack source when it is created or rotated; never returned by the API; the rotate endpoint updates it; migration none (the column exists). Sources made before this have no copy: the action tells the admin to rotate the token once.
- [ ] gRPC calls in `grpc_api.py`: `get_http_integration(application_id)` (None when ChirpStack answers not found), `create_http_integration`, `update_http_integration`, on the existing channel and token, with the same error translation as the other calls.
- [ ] `ChirpStackManagement.connect_applications(webhook_url, token)` (D125): per application of the tenant, read the integration; none: create with our URL and JSON encoding; exists: append our URL when absent (comma list, trimmed), keep the other URLs and every header, set nothing else; already there: report as connected. Returns per application: id, name, outcome (`connected`, `already`, `failed`), the URLs after the change, the error. Never removes a URL or a header.
- [ ] `ChirpStackManagement.integration_status()` for Test connection: per application whether our URL is in its HTTP integration, another URL is there too, or there is none; the summary counts. Test connection's result shows "12 applications, 9 connected" and the names of the rest.
- [ ] API: `POST /data-sources/{id}/connect-applications` (server admins, the api channel on, ChirpStack only; 409 with a plain message when the source has no token copy), audited with the outcome per application; the `ConnectionTestResult` gains `applications` for ChirpStack.
- [ ] Frontend: a "Connect applications" button next to Sync devices and Sync gateways on the data sources page for ChirpStack sources with the API channel on; a result dialog listing the applications with the outcome and the reason; Test connection's dialog shows the application status; the create dialog for a ChirpStack source offers the button right after saving, when the token is shown.
- [ ] Docs: `docs/integrations/chirpstack/index.md` (the one-step setup, the token in the URL, what stays manual), `DEVELOPERS.md` (the connector calls, the encrypted copy), the changelog; ADR when the token copy is judged worth one.
