...
- [ ] Traffic columns per channel: traffic rows carry `acquisition_channel` and `ingestion_method`; the project, source and server traffic pages show port, FCnt, SF, RSSI, SNR and gateways only for LoRaWAN rows, and for Bluetooth, log file and Iridium rows the frame port, the log file or session, the frame length and the satellite session time; a Channel column and the per-channel details in the row expansion; the device page's traffic section (below) uses the same component.
