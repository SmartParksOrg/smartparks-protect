...
| Next item | Phase 16 part A is built and waits for its live check on the dev server (then tick its boxes); part B, the all-projects scope, follows; v2.0.1 with the fixes since v2.0.0 on Tim's word, or fold them into v2.1.0. Open: Request status through KPN for the complete timeline, Sync gateways and the four DevEUIs on ChirpStack, the live verification stages that wait for other accounts |
