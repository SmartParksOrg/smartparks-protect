...
| Next item | Phase 15 in order: assignment dates and the attribution repair (SP051307's log records as the acceptance case), then device health and last seen, traffic columns per channel, gateway metadata, the firmware-aware driver, the metrics review; then getting around, organising, presentation, coverage, and v2.0.0. Also open: Request status through KPN for the complete timeline, Sync gateways and the four DevEUIs on ChirpStack, the live items that wait for other accounts |
