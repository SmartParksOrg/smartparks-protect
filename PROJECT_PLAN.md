...
| Field | Value |
| --- | --- |
| Active phase | Phase 14 complete except the v2.0.0 release (CI green, dev server on ad70957). ChirpStack round trip proven both ways on 2026-09-05 and confirmed on 2026-09-06: Request status and Reset for SP051307 reached confirmed by device, Set GNSS interval transmitted. KPN LoRa live run in preparation (D95 built on 2026-09-06, deployment and the KPN portal steps next) |
| Latest release | v0.6.0 (2026-09-04): phases 7, 8 and 9; phases 10 to 13 and the deployment fixes unreleased |
| Last session | 2026-09-06 |
| Next item | KPN LoRa live run (stage 1 of `docs/operations/live-verification.md`): commit and push D95, redeploy the dev server, create the KPN data source in project Smart Parks, Tim configures the application server in the KPN Device Manager (webhook URL, security with AS ID and key), first uplinks in Data sources, Traffic, then Request status through KPN; record the first pushes as fixtures and fix the deep link template. Then Sync gateways on ChirpStack and the four DevEUIs in Needs attention; v2.0.0 on Tim's word; decide whether a fresh install seeds the OpenCollar device type; the ChatGPT half of phase 9 and the other live items follow the accounts |
