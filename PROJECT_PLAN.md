...
| D95 | ThingPark push authentication | The webhook verifies ThingPark's own `Token` (SHA-256 over the report's body elements, the URL query in ThingPark's order and the tunnel interface authentication key) with the source's `as_key`; the bearer header stays the alternative; adapters may declare `verify_webhook`. Downlinks carry a 64-bit `CorrelationID` from the command id so the sent report moves the command to transmitted | KPN's Device Manager may not allow custom headers, and the same key already serves downlinks; ThingPark matches reports on the CorrelationID, the response body carries no reference. Built from Actility's documentation and published examples (golden fixtures). Decided by Tim on 2026-09-06. |
| D96 | Bulk onboarding from Needs attention | Checkboxes on the unknown identities with select-all in the filter; "Create devices" makes a device per selected identity with one type, an optional project (assigned from the first sighting) and an optional entity type (an entity of the same name per device, assigned from the same time); names from the platform's device name kept on the identity (`name` attribute, KPN fills it from ThingPark) else the external id; "Ignore" for the rest; skips reported per identity | A shared KPN application server posts every collar of the subscriber (77 unknown DevEUIs on the first day); one by one does not scale and the CSV import needs the DevEUIs typed. Decided by Tim on 2026-09-06. |
| D97 | Placement of the live-day observations | A new phase 15 (correctness and usability from the first live days) before the v2.0.0 tag; phase 14's release box moves there | 2.0 is the production milestone; a park should meet the product after these fixes, not before. Decided by Tim on 2026-09-06. |
| D98 | Groups | Folder-like groups per project: `entity_groups` with an optional parent, nested as deep as needed, an entity in at most one group, devices grouped through their entity; the lists filter by group and the map shows and hides per group | Simple to explain and to render as layers; tags stay possible later for ad hoc filtering. ADR 0020. Decided by Tim on 2026-09-06; amended the same day from two levels to any depth after trying it against EarthRanger's groups. |
| D99 | Site-wide search | A command palette (Ctrl+K and the header search box, cmdk) over entities, devices, features, gateways, data sources, projects and pages within what the user may see, backed by one bounded `GET /search` endpoint; lists keep their own search box | One place to type anything; the component is already in the app. Decided by Tim on 2026-09-06. |
| D100 | Firmware-aware OpenCollar driver | Layouts per firmware range in the driver from research section 5 (ports 8 and 17 up to 6.16, 13 byte CMDQ records before 6.9, signed multi-byte fields before 6.13, ports 21, 27 and 28 from 7.2), selected from the device's firmware version learned from its status messages and stored on the device (or the log's); the layout used is recorded as the decoder version; golden tests against the versioned raw log decoders | Older collars in the field send layouts the 7.3 decoder misreads; the differences are documented and small. ADR 0021 when built. Decided by Tim on 2026-09-06. |
| D101 | Serial number | The manufacturer's serial printed on the hardware; for OpenCollar the DevEUI, filled automatically from the first LoRaWAN identity when empty; the name stays the human name | One meaning for the field; nothing to type for collars. Decided by Tim on 2026-09-06. |
...
| D119 | Device clocks ahead | A record whose device time runs more than an hour ahead of its delivery is stored and marked invalid (off the map, the lists, the rules, the exports and the current state), the trace step says by how much, Needs attention lists the devices, the device page warns, and the fix is the bulk curation job with a time offset (reason Device clock error), whose bound grew to twenty years; the existing rows of SP051479 shifted by such a job | A collar with a wrong clock epoch sent positions dated 2030 and its current state could never move again; nothing is guessed and nothing is lost (architecture 28). Decided by Tim on 2026-09-07. |
| D120 | Devices in no project | In the all scope a server admin sees the devices that are in no project as a "Not in a project" section of the device layer and the Devices tab (foldable, switchable, off by default), with their latest position and, in that scope only, a track over the positions attributed to no project; the panel says so and links to Server admin, Devices; the all-scope devices list gets a Project filter with "Not in a project" | Inventory, workshop and freshly onboarded hardware belongs to nobody yet, and the person running the server wants to see where it is next to the parks; unassigned data is theirs to see until attributed (architecture 28.11). Decided by Tim on 2026-09-07. |
| D121 | Reprocessing after linking runs in the decoder | Linking an identity to a device queues its retained events (received, with the device) in one statement and sends the decoder one message per identity; the decoder walks them oldest first, each committed on its own; the request returns at once. Needs attention shows a Processing card with a progress bar from the summary's count of queued events, polled every few seconds, and a toast when the queue drains; the user can leave. The receptions of those uplinks get the device at the same time. | A bulk create of eighty collars republished thousands of events one by one inside the request, with the user watching a spinner; the decoder owns processing anyway, and a walk over the queue is durable and idempotent where a background task in the API is neither. Decided by Tim on 2026-09-07. |
| D122 | Bulk assignment of devices to a project | In All projects the devices list has a selection with Assign to project: one project for the selection, each assignment from the device's first data (records, identity first seen, log files) or from now, an optional entity per device named as the device with a group, records in the range reattributed, devices assigned elsewhere in the range skipped with the reason. Server admins only. | Seventy-six collars onboarded without a project had to be assigned one by one; the bulk create already offered the same choices, and the first data is the start the device page itself proposes (D103). Decided by Tim on 2026-09-07. |
| D123 | Heard positions as points at every zoom | The coverage layer draws the heard positions as coloured dots at every zoom, the newest 10,000 in view when there are more; the hexagon aggregation of D107 stays in the API on request only. | The hexagons were computed in the database per viewport and lagged on every pan, while a few thousand dots draw at once on the client and read better; Tim preferred the dots on 2026-09-07. |
| D124 | A small map on the entity and device pages | A card that is nothing but a still MapLibre map: the newest position marked and the last ten positions as a thin trail, fitted to the trail, no gestures, the whole map a link to the live map on the object. | Someone reading an entity or device page wants to see where it is without opening the live map; a still preview keeps the page scrolling on a phone and stays light. Decided by Tim on 2026-09-07. |
...
| D142 | Records shape | One row per device timestamp with the position and every measurement and state field of that moment, from one keyset query over the union of position and measurement times, newest first, 1,000 per page with a `time,device` cursor and a separate count for the progress bar, in the all scope too (`GET /projects/{id}/records`, `/records/count`) | The data arrives that way (a fix with its coordinates and the metrics of the moment) and that is how people read it; the bound per request holds (13.10). Decided by Tim on 2026-09-09. |
| D143 | Never a wall | The records view loads pages in sequence into a virtualized table with a progress bar, the count and a Stop button, charts redraw as pages arrive, and above a size it suggests an export job but keeps loading; every limit that refused before answers with a page or a coarser bucket and says so | A bound on a request is an architecture rule, a wall in front of the user is not. Decided by Tim on 2026-09-09. |
| D144 | Records export with an automatic job | A `records` dataset (wide by default, long as an option) in every format, started from the records view's selection; the dialog counts first, downloads directly under `DIRECT_MAX_ROWS` and above it starts the job by itself and shows its progress and the download inline | The person asked for the data, not for a choice of mechanism. Decided by Tim on 2026-09-09. |
| D145 | Records first, Analysis kept | The explorer opens on a Records tab: the object picker (entities and devices, several, search, the group as a shortcut), the period (presets, custom, an entity's assignment period), the columns (every metric with data, all on, a picker per user); today's explorer is the Analysis tab with "Analyse these" carrying the selection over; saved views carry a mode; the Data tabs of the entity and device pages open the records view for that object | The simplest question first, the analysis one click further. Decided by Tim on 2026-09-09. |
| D146 | Charts in the records view | One small chart per numeric column over time for the selection, click to enlarge, and the track of the loaded rows on a small map, all from the loaded rows, behind a "Charts and map" toggle that starts off; the first look is the table alone; the Analysis tab keeps the big ECharts views | The records view stays the simple first look. Decided by Tim on 2026-09-09, amended the same day after seeing it: the charts and the map cluttered the first view. |
...
### Phase 15: correctness and usability from the first live days (v2.0.0)

**Goal.** What the first days with real collars on the dev server showed (Tim's observations of 2026-09-06, session log): records that exist but stay invisible, device health that never reaches the screens, LoRaWAN columns on Bluetooth and satellite traffic, older firmware decoded with the newest layout, and pages that show the machinery before the animals. Fix what is wrong or hidden, then getting around, then organising, then the clean-up; the coverage tool last; v2.0.0 at the end.

**Why here.** 2.0 is the production milestone and a park should meet the product after these (D97). The order (D108) is correctness first because every later screen builds on health, attribution and groups.

**Deliverables.**

Correctness and visibility:

- [x] Assignment dates and the repair (D103) (2026-09-06, `reattribute` in `shared/domain/assignments.py`, `GET /devices/{id}/data-span`, `extend-start` on both assignment kinds, the date-choice field in the admin device dialogs, the warning with the repair on the device page; creating an assignment or a handover attributes the records already in range as well; the entity page's dialog arrives with D106; the SP051307 repair on the dev server follows the deploy): the assignment dialogs (device to project and device to entity, on the device page and the entity page) offer "since the device's first data" (earliest record or sighting across its identities and log files), "since it joined the project", "from now" and a date; date-only with the start of the day in the project's timezone. A device page whose device has records before its earliest project assignment says so ("N records before <date>") and offers "Extend the assignment back to the first data", which moves the assignment start and reruns attribution over those records (the curation recompute path generalised to `reattribute(device_id, from, to)`); the same for entity assignments. Acceptance: SP051307's 29,253 measurements and 223 positions from the Bluetooth log (July 16 to September 5, no project today) become visible in project Smart Parks.
- [x] Device health (D104) (2026-09-06, `HealthField` on drivers, `OPENCOLLAR_HEALTH`, `shared/domain/health.py`, migration 0016 with `latest_measurements` and `latest_state_time`, firmware written on the device, `with_state` on device reads, health on map features, the Health card and list line; the entity page's line arrives with D106): drivers declare `health` (keys from state and measurements with label, unit, thresholds and the order to show them); OpenCollar from the port 4 status (battery and charging, temperature, uptime, reset count, error flags, firmware and hardware version, last status time) and the last fix (satellites, accuracy, time to fix). The device read carries `health` built from `DeviceCurrentState.latest_state` and the latest measurements; a Health card on the device page, a compact health line in the devices list, the entity page and the map selection panel. Last seen is the latest record of any kind everywhere (the current state already has it; the lists and the map must read that). `Device.firmware_version` is written from every status message (the column exists and was never written).
- [x] Traffic columns per channel (2026-09-06, `TrafficRow.acquisition_channel`, `ingestion_method`, `frame_bytes`, `delivered_at`, `log_file_id`; `TrafficTable` with a Details column per channel on the three pages; the device page's section arrives with the next group): traffic rows carry `acquisition_channel` and `ingestion_method`; the project, source and server traffic pages show port, FCnt, SF, RSSI, SNR and gateways only for LoRaWAN rows, and for Bluetooth, log file and Iridium rows the frame port, the log file or session, the frame length and the satellite session time; a Channel column and the per-channel details in the row expansion; the device page's traffic section (below) uses the same component.
- [x] Gateway metadata kept whole (2026-09-06, per-gateway coordinates normalised to `location` by LORIOT and The Things Stack as ChirpStack and KPN already did, `location_source` and `location_at` on the registry with migration 0017, an admin position kept over platform updates): every reception keeps the platform's gateway fields (KPN: signal, chain, and the best LRR's coordinates today; ChirpStack: location, metadata, uplink id; The Things Stack: location), the registry keeps a location per gateway with its source and time, and a reception carries its own gateway's coordinates when the platform gives them per gateway. The basis of the coverage tool.
- [x] Firmware-aware OpenCollar driver (D100, ADR 0021) (2026-09-06, `LAYOUTS` and `layout_for`, `SourceEventData.firmware_version`, in-stream learning in flash logs, ports 8, 17 and 21 decoded per layout, migration 0018 for the air quality metrics, vendored decoders with `scripts/opencollar_golden.py` and the golden test; the wire formats turned out identical across versions, only the message set and one status bit differ): per-firmware layouts from research section 5, the firmware learned from status messages and stored on the device (or taken from the log), `decoder_version` naming the layout used, golden tests with the outputs of the versioned raw log decoders on SP051307's log and the wiki examples; ports 21, 27 and 28 decoded or noted per version.
- [x] Metrics review (D102) (2026-09-06, `GET /attention/metrics`, the New metrics tab and stat, the explorer's marker): a Needs attention tab "New metrics" listing `uncategorized` metrics with inline label, unit and category; the Data Explorer marks them until defined.
...
- [x] Entities and devices (D106) (2026-09-06, `EntityPage` at `/projects/{id}/entities/{id}` with the Device, Health and Assignments cards, `AssignDeviceDialog` over `GET /devices?project_id=&unassigned=true` and the guided start field, "Release device" through the existing end-assignment call, `device_name` and `entity_name` on assignment reads, the list row, the map panel and the device page link to it): the entities list shows the assigned device with a link; the entity page gains an Assignments section with the history and "Assign device" from the project's unassigned devices through the guided dialog.
...
- [x] (2026-09-07, commit 19677ec, live on the dev server) API `GET /projects/{id}/map/devices` (D111): devices with a project assignment valid now, `device_current_state` latest position and time, last seen, health, the entity tracked today (`entity_id`, `entity_name`, `group_id`), `project_since` (the assignment's start, for the "since assignment" track length), device type key and icon, `picture_updated_at`; the same `bbox` and `limit` bounds as `map/current`; devices without a position come back without geometry so the panel can list them. Tests: an unassigned device with a position appears, a device whose assignment ended does not, a viewer of another project gets 403, the bound holds.
- [x] Map layers (`layers.ts`): a `devices` source with the square marker family and the device type icon, health colour as for entities, the name as label; `bindDeviceClicks`; `setDevices`. Device tracks (D114) on the track source with a dashed line style and the device's colour; `trackFrom` for a device uses `project_since` for the assigned length.
- [x] Layer choices (D113): `shown_devices` as a shown set with `showDevice`, `hideDevice`, `showAllDevices`, `hideAllDevices`, `isDeviceShown`; `?device_tracks=` next to `?tracks=`; unit tests in `layerChoices.test.ts` including "a new device is off".
- [x] Devices tab in the layers panel: search, sort by name or last seen, Unassigned heading first then devices under the entity they track, a row with the icon or picture, last seen, the track toggle and locate, Show all and Hide all; the badge counts shown devices; the Tracks card counts entity and device tracks.
- [x] Device panel on the map (D112) with `?device=`: name, type, last seen, the health line, the entity it tracks with a link (or "no entity"), open device, Show the track; opening it closes the entity panel and the other way round; on a phone the map moves the device above the panel as for entities.
- [x] Getting there: the device page's "Show on map" works for a device without an entity (the map with the device shown and selected), the devices list's map link does the same, the search palette's device hit too.
- [x] Docs: the live map row in `docs/administration/pages.md`, the map section of `DEVELOPERS.md`, the changelog.
...
**Goal.** A ChirpStack tenant with many applications connects to the platform in one step: the data source, made once with the tenant API key, puts its webhook on every application's HTTP integration, shows which applications are connected, and connects a new application on request.

**Why here.** Tim ran into it while adding ChirpStack sources on 2026-09-08: the API integration is per tenant, the HTTP integration per application, so every application meant opening ChirpStack and pasting the same URL and header. The gRPC API the adapter already uses carries the calls to do it from our side.

**Order (D125 to D127).** The webhook side first (the token in the query for ChirpStack, the encrypted copy), then the connector calls and the action, then the status in Test connection, then the frontend and docs. Each item lands with tests and docs, committed on Tim's word.

**Deliverables.**

- [x] Webhook: `webhook_token_in_query` on the ChirpStack adapter (D127), so `?token=` authenticates as the Authorization header does; the token in the query never reaches the audit or the traces. Test in `tests/api/test_ingest_and_attention.py` or a new `test_chirpstack_connect.py`.
- [x] The encrypted copy (D125): `webhook_token` kept in `credentials_encrypted` of a ChirpStack source when it is created or rotated; never returned by the API; the rotate endpoint updates it; migration none (the column exists). Sources made before this have no copy: the action tells the admin to rotate the token once.
- [x] gRPC calls in `grpc_api.py`: `get_http_integration(application_id)` (None when ChirpStack answers not found), `create_http_integration`, `update_http_integration`, on the existing channel and token, with the same error translation as the other calls.
- [x] `ChirpStackManagement.connect_applications(webhook_url, token)` (D125): per application of the tenant, read the integration; none: create with our URL and JSON encoding; exists: append our URL when absent (comma list, trimmed), keep the other URLs and every header, set nothing else; already there: report as connected. Returns per application: id, name, outcome (`connected`, `already`, `failed`), the URLs after the change, the error. Never removes a URL or a header.
- [x] `ChirpStackManagement.integration_status()` for Test connection: per application whether our URL is in its HTTP integration, another URL is there too, or there is none; the summary counts. Test connection's result shows "12 applications, 9 connected" and the names of the rest.
- [x] API: `POST /data-sources/{id}/connect-applications` (server admins, the api channel on, ChirpStack only; 409 with a plain message when the source has no token copy), audited with the outcome per application; the `ConnectionTestResult` gains `applications` for ChirpStack.
- [x] Quick setup (D128) and the preview before Apply (D129): `quick_setup` and `complete_config` on the adapter, `list_tenants`, the form's two fields with Advanced folded, `?dry_run=true` with the before and after per application; tests for the derivation, the refusals and the dry run.
...
- [x] Tabs on the entity page (D133): Overview (entity card, the small map, the device and its health, assignments, the repair callouts), Data (positions and events of the entity with the curate, deliveries and source-event links always shown), Network (the traffic and traces of the device tracking it, with the link to the device). `?tab=` in the URL, Overview when absent; the existing deep links (`?event=`, the assignment dialogs) keep working.
- [x] Tabs on the device page (D133): Overview (device card, the small map, health, project and entity assignments, actions, the repair callouts), Data (recent positions and records with their links), Network (external identities, traffic, traces, commands and the request buttons). The Traffic and Trace explorer pages link into the device's Network tab where they name a device.
- [x] The switch goes (D134): `useTechnicalDetails`, `TechnicalDetails`, the sidebar switch and its strings, the `technical` flag on navigation sections replaced by the role rule (`adminOnly` for the Network section), `sectionsFor` tested for a viewer, a project admin and a server admin; the `technical_details` preference key is left unused on the server (no migration; the preference document tolerates unknown keys).
- [x] Lists: the devices list's Driver column becomes a column the picker can show (hidden by default, `defaultHidden`), no longer switch-bound; the entities, devices, gateways and traffic lists audited once more for columns a viewer does not need first.
...
- [x] (2026-09-09, `heat.ts` with tests, `HeatSettings.tsx`, the strip button, the card and the settings in the right column; checked against the dev API with the points stubbed from the real tracks at 1440 and 390 px; reworked the same day on Tim's correction: per entity and device like tracks, `?heat=` and `?device_heat=`, a toggle next to the track's on every row and panel, the strip button folds the card) The heatmap layer in `layers.ts` (`ensureHeatLayer`, `setHeatPoints`), radius in metres to pixels per zoom, sensitivity as intensity and weight, hidden below the shown layers; the Heatmap card (`HeatSettings.tsx`) with the sliders (radius 10 m to 5 km, sensitivity Low to High, look-back 1 hour to 90 days with the quick picks), the URL parameters and the per-user default (`heat_settings` in the preference document); the objects it covers are the shown entities and devices, or the selected one when the card says so.

d. Drawing and measuring:

- [x] (2026-09-09, terra-draw 1.33.0 with its MapLibre adapter 1.4.1 per D139, `draw.ts` with a `DrawSession`; the Features page's dialog draws with it; the session is exercised in the browser check rather than a unit test, the library owns the editing) Shared drawing module (`components/map/draw.ts`) from the Features page's `DrawMap`: point, line and polygon, vertex add, move and delete, escape cancels, enter finishes; the Features page uses it too. Unit tests over a fake map.
- [x] (2026-09-09, `DrawBar.tsx` with the running measure and `SaveFeatureDialog`, the Draw button for project admins, the created feature selected on the map) Draw mode on the live map: the Draw button starts it, a small bar says what to draw and shows the running length or area, finishing opens "Save as feature" (name, type; geofence, site, zone or route) through the existing features endpoint, the feature appears in the Features tab of the layers panel at once.
- [x] (2026-09-09, `FeaturePanel.tsx` and `featureTools.ts`, `?new=<template>&feature=<id>` on the rules page, `NearCondition` with `metres_between` and `metres_to_geometry`, `entity_points` on the data access, the `near_site` template, the builder's near row, tests in `tests/shared`) From a feature to a rule: the feature panel (a click on a feature on the map, the same panel component) and the save dialog offer "Create rule": the rules editor opens with the geofence enter or exit template and the feature preselected; the reserved `near` condition (proximity, D45) enabled if the decision says so, with its evaluator, template and tests.
- [x] (2026-09-09, the Measure button for everyone, `lib/geodesy.ts` tested against known values) Measure mode: distance along a line and area of a polygon shown live while drawing and in a small result card with copy; `lib/geodesy.ts` (haversine distance, spherical polygon area) with tests against known values; nothing is saved unless "Save as feature" is chosen.
...
- [x] Phase 19: a MapTiler Cloud key for the dev server (2026-09-09, Tim's key in the vaulted host vars as `maptiler_key`; compose had to pass `MAPTILER_KEY` to the API service, 7d97ff5; Satellite and 3D terrain seen live).
...
### 2026-09-05, ChirpStack round trip and what the day taught (Claude and Tim)

- The gRPC direction is live: after the nginx location on chirpstack-dev4 and a tenant API key, Test connection answered, Sync devices listed the tenant, and a Request status for SP051307 was queued in ChirpStack's device queue (command `queued` on our side since 14:03 UTC; the collar's next uplink collects it). The nginx `grpc_pass` location first locked everyone out of ChirpStack's web UI, because the UI talks grpc-web on the same `/api.` paths: the documented location now routes on the content type (native `application/grpc` from the Protect server to `grpc_pass`, everything else to the plain proxy), checked against nginx 1.18 and ChirpStack 4.19.1, with the alternatives for direct port 8080, a separate TLS port and Caddy.
- Fixes found by the live check: the adapter labelled HTTP integration events as MQTT, so the HTTP channel counted nothing; a proxy answering a plain HTTP status is now explained as "grpc_pass missing or not reloaded" instead of COMMAND_REJECTED; every list table has a search box and Devices and Entities search on the server, because the dev server's five thousand benchmark devices hid the new collar behind the 500-row cap, which the page now announces; the runbook says how to create the OpenCollar device type on a server without the demo bootstrap and that a device from Needs attention needs a project assignment before the project's Devices page lists it.
- Committed: 67386fd (form per channel, migration 0015), ad70957 (search, channel counting, proxy error); the nginx documentation fix is committed at the end of this session. Dev server refreshed to ad70957.
- Where to start next time: check Server admin, Traffic, Commands for the Request status of SP051307 (sent after the collar's next uplink, then a status uplink with the answer); if it stays queued, look at ChirpStack's device queue and events; then the items under "Next item" above.

### 2026-09-06, KPN LoRa preparation (Claude and Tim)

- The dev server showed the ChirpStack round trip complete: Request status (14:03 UTC) and Reset for SP051307 reached confirmed by device, a Set GNSS interval was transmitted; the plan's next item is closed.
- Tim wants the KPN LoRa integration tested live on the dev server: his account is on KPN's ThingPark (Actility), a collar is registered and sending, he holds an AS ID and AS key, the work is split (Tim in the KPN portal, Claude on the server). Decisions asked and taken: verify ThingPark's own push Token instead of relying on a custom header (D95), send a CorrelationID with every downlink, the collar joins project Smart Parks next to SP051307, build first and connect after.
- Read before building: the ThingPark tunnel interface pages (report types, body elements of the Token per report on the ThingPark X HTTP connector page), Actility's published `verify_token_in_uplink.py` and `send_downlink_with_token.py` (the query order and Time format of a downlink, the 64-bit CorrelationID), IoT Academy's hookbin capture of a real KPN push (KPN's downlink URL `api.kpn-lora.com`, the `+02:00` in the query, the AppSKey ThingPark can forward), Device Manager's security activation screen (AS ID, tunnel interface authentication key, timestamp deviation). Both published tokens recompute exactly with the recipe.
- Built: `verify_webhook` on the KPN adapter (per-report body elements, raw query with `+` preserved, constant-time comparison, a warning on mismatch), the endpoint hook after the bearer check, `CorrelationID` in the downlink query inside the token and as `provider_ref`, `queue_ref` from the sent report's CorrelationID (upper case), the best LRR's coordinates on the reception, KPN's downlink URL as the example, channel hints, two sourced fixtures, golden and API tests, the runbook rewritten with the Device Manager steps.
- Open: commit and push on Tim's word, redeploy, then the live steps; the `web_url` deep link template is a guess until Tim copies a device URL from the portal.
- Live, 06:40 UTC: after Tim set the route's strategy to Blast (Sequential posts only to the first destination that answers), ThingPark's pushes arrived and verified with the key of the existing application server (AS ID `ASIDsmartparkseu`, security already active for the Node-RED flow): eighteen uplinks in the first minutes from the other collars of the KPN application, decoded as port 4 status frames with two gateways each, all in Needs attention as unknown DevEUIs. SP051307 (identity added on the KPN source next to its ChirpStack and WebBLE identities) joins one network at a time by its AppKey; Protect accepts the same DevEUI from several sources by design. Tim asked for a simpler source form: the bearer downlink mode is removed, one `as_key` and the `as_id`, KPN's downlink URL as the default.
- Tim tested both directions over KPN and asked what to optimise. Checked on the dev server: 469 pushes, all accepted with the Token verified (ThingPark sends the `+` of the offset percent-encoded); SP051307 decoded fourteen times over KPN with its track on the map; 17 KPN gateways with coordinates in the registry; a Set GNSS interval through KPN accepted and, 39 seconds later, transmitted through the sent report's CorrelationID (`DeliveryStatus` 1, RX1 cause C0 "LRC selected RX2"); 77 unknown DevEUIs from the shared KPN application in Needs attention. Fixed from the findings: `DeliveryStatus` 0 fails the command with the causes translated (Wireless Logger documentation), the frequency on the traffic row, the downlink channel status from the last command, the doubled slash in the KPN deep link, port 0 and unknown ports (port 199, the legacy Modem-E message three collars send) as trace notes instead of dead letters (the two dead letters from ChirpStack yesterday were port 0). Open: the KPN device deep link path (the Device Manager may have none), and a bulk "create devices" action in Needs attention for a shared application (not built).
- Committed and pushed as e44874d (CI green), dev server redeployed. Tim: the KPN Device Manager has no per-device URL (the legacy portal sits behind a portal link), so the deep link stays as it is for now; bulk actions in Needs attention are wanted (D96, asked and taken: devices with an optional entity each, platform names with the DevEUI as fallback, checkboxes with select-all in the filter). Built: `bulk-create-devices` and `bulk-ignore`, the `name` attribute on KPN identities from ThingPark's device name, the selection column on the shared table, the toolbar and dialog on the Needs attention page, tests, docs.

### 2026-09-06, observations from the first live days and the plan for phase 15 (Claude and Tim)

- Tim's observations after a day of use (in his order): traffic views always show LoRaWAN columns; the Bluetooth log download of SP051307 looks wrongly processed; non-position data (the port 4 status) does not update the device; a device traffic section on the device page; links to the live map from devices and entities; too much technical detail for general users; a distinction between what admins and others see; keep the gateway metadata of LoRaWAN uplinks for a network map and coverage analysis (a small ttnmapper-like tool); a map button for the browser's location and live "ago" values; devices visible and assignable from the entities view; valid_from that guides the user; the purpose of the serial field; groups and subgroups for entities and devices; a site-wide search; hiding entities per group on the map; automatic detection of new metrics.
- Checked before planning: the log was decoded correctly (33,471 records, firmware 7.2, July 16 to September 5), but 29,253 measurements and 223 positions carry no project because the collar joined project Smart Parks on September 5 12:03; the attribution, not the decoder, hides them. The research document already lists the decoder differences per firmware (section 5), so firmware awareness is layout selection, not research. `raw_logs_decoder` ships one decoder per firmware range and `field-meta.json` with the field groups; `smartparks-connect-web` is a settings manager over ChirpStack and RockBLOCK, not a metrics view, so the health fields come from the status message and the research document. Metrics already register themselves (`uncategorized`); the ask is their review.
- Twelve decisions asked in three rounds, every recommendation accepted: D97 (phase 15 before 2.0), D98 (folder-like groups), D99 (command palette), D100 (firmware layouts in the driver), D101 (serial is the DevEUI for OpenCollar), D102 (metrics keep registering, reviewed in Needs attention), D103 (assignment dates from the data plus the repair), D104 (health declared by the driver), D105 (operational view first, technical details on request), D106 (assign devices from the entity), D107 (gateway map plus coverage layer), D108 (correctness, navigation, organising, presentation, coverage).
- Written: phase 15 with its deliverables, the milestones table (v2.0.0 at the end of phase 15), the status block. Nothing built yet; the first item is the assignment repair.

### 2026-09-06, phase 15 item 1: assignment dates and the attribution repair (Claude)

- Built `reattribute(session, device_id, start, end)` next to `resolve_attribution`: clears and rewrites the project and entity of the device's positions and measurements in the window from the assignments as they stand (effective time, compressed chunks decompressed in the transaction, current state recomputed). `GET /devices/{id}/data-span` gives the first record, first sighting, first log and the earliest assignments with the counts of records before them; `POST /devices/{id}/project-assignments/{id}/extend-start` and `POST /projects/{id}/entity-assignments/{id}/extend-start` move a start back and attribute the records in between (the entity one refuses while the project does not cover the new start). Creating a project or entity assignment and a handover attribute the records already inside the new range, which the test found missing: a record decoded before its assignment existed stayed without entity forever.
- Frontend: `AssignmentStartField` (since the device's first data, since it joined the project, from now, on a date as the start of that day in the project's timezone) in the admin device dialogs; the device page warns about records before the earliest project or entity assignment and offers the repair to project admins and server admins.
- Verified: ruff, mypy, the API and decoder suites (the new repair test walks the whole story), frontend lint, types, catalogue and tests, docs build and check. Not yet run on the dev server: the SP051307 repair itself.

### 2026-09-06, phase 15 through the first half of getting around (Claude)

- Done and deployed today, each item committed on Tim's word with CI green and the dev server refreshed: assignment dates and the attribution repair (SP051307's log records are visible in project Smart Parks since the repair ran live), device health and last seen (migration 0016), traffic columns per channel, gateway positions with their origin (migration 0017), the firmware-aware OpenCollar driver with golden tests against the vendored reference decoders (migration 0018 for the air quality metrics, ADR 0021), the metrics review tab, the device page's traffic section and map links, the entities list's device links, the map's geolocate control, live "ago" values through `useNow`, and the DevEUI as the OpenCollar serial (commits 26b2411, 11ba4a6, 71bbea8, 34391aa, df82d3b, 5961576, afbc6f4).
- Findings worth keeping: creating an assignment or a handover now attributes the records already inside the range, which the repair test found missing; the wire formats of every OpenCollar firmware are identical, only the message set and one status bit differ; an administrator's gateway position used to be overwritten by the next reception.
- Where to continue: the entity page (D106) is the next unchecked item; the remaining groups follow in the order of D108. The session was compacted after this entry.

### 2026-09-06, the entity page (Claude)

- Built the entity page (D106): the Device card with the current device, its health and "Release device", the Assignments history with device names, "Assign device" from the project's unassigned devices (`unassigned` on the devices list) through the guided start field; the entities list, the map panel and the device page link to it. Assignment reads carry `device_name` and `entity_name` where the reader needs them.
- Verified: ruff, mypy, the API, shared and decoder suites with the new entity page test (a project admin assigns, releases, a viewer is refused), frontend lint, types, catalogue and tests, docs build and check.
- Where to continue: organising (groups, D98), then map layers per group and site-wide search (D99).

### 2026-09-06, groups (Claude)
...
- Tim's bug: Hooglander Horsterwold 0253 assigned to SP051440 "from now" (18:03:45) showed no last seen and no position while the collar had traffic. Cause one: nothing had arrived since the start, by design. Cause two, real: `recompute_current_state` skipped entities without a current-state row, so the repair could never put a fresh entity on the map; fixed, with the repair test asserting the map feature. The entity page now warns about records before the assignment and offers the repair; the dialog explains the start choice.
- Tim's second look found the real cause of the "from now" start: `AssignmentStartField` showed "since the device's first data" but only reported a choice on click, so the dialog submitted its own default. Fixed at the field, which now reports its first choice. Also asked: change an assignment instead of only removing it (`AssignmentChange` on the PATCH, the Change… dialog on the entity page), the entity's traffic windowed to the assignment, and KPN's join notification recognised as a join (live fixture `kpn_live_join_notification.json`; source event 5121 on the dev server stays a log line, it was stored before the change).
- Where to continue: plan the next phase with Tim from the open items; these fixes go out as v2.0.1 when he says so.

### 2026-09-07, map clicks after a project switch (Claude)

- Tim found on the dev server that clicking an entity on the live map sometimes moved him from project Smart Parks to Demo. Cause: the map's click handlers were bound once per map (inside `ensureEntityLayers`) with the first project's `select`, whose `setSearchParams` resolves against the pathname it was created with; the map and the page outlive a project switch on the map page because only the route parameter changes. So after opening Demo first and switching to Smart Parks, a click wrote `?entity=` onto the Demo URL. Fixed by splitting layer creation (`ensure*Layers`, once) from click binding (`bindEntityClicks`, `bindEventClicks`, bound in an effect with a cleanup that unbinds), with a unit test over a fake map; the map also fits the new project's entities after a switch, which the same stale page had skipped. The dashboard's map tile uses the same binding.
- Housekeeping: the session log had the entries from site-wide search to Tim's first hour twice (a compaction artefact); the duplicate block is removed and the two entries between them (map layers reworked, organizing page) now sit in date order.
- Committed as 0def568 on Tim's word and pushed.

### 2026-09-07, track settings on the map (Claude)

- Tim asked for EarthRanger's Track settings on the live map: a dynamic track length when one or more tracks are on. Four decisions asked, D109 records them: a Tracks card at the top of the map while tracks are on ("2 tracks, 143 points over 21 days", gear, Clear tracks), a slider in days plus a number field in hours or days with the quick picks, "since the device was assigned to the entity" next to the custom length (Tim's choice over custom only), and the URL plus a per-user default.
- Built: `assigned_since` on the map's current-state features (the start of the entity's current device assignment, one query per response), `trackLength.ts` with tests, `TrackSettings.tsx` (card and panel on the shadcn slider and radio group, added with the CLI, which also pulled a stray `cn` package that was removed again), the map page reading `track=` as hours or `assigned` with the preference `track_length` as the default, the selection panel's period list replaced by a Show track button, and the layer panel's tooltip naming the current length.
- Verified: frontend lint, types, catalogue, 29 tests and the build; ruff and mypy on the API. The map API test with the `assigned_since` assertion could not run here (Docker Desktop was off); CI runs it. Committed as 7b74101, pushed, dev server updated with the playbook (env-refresh, all checks green).
- Tim asked for a visual inspection at phone width. The screenshot sweep against the dev server (58 routes, three widths) found no console error or page overflow, but the sweep's first project is the benchmark park and it never switches a track on; extra phone screenshots of project Smart Parks with two tracks on showed the Tracks card widening the map's top bar past the screen, so the page scrolled sideways and the settings panel landed off screen. Fixed: the top controls are bounded on the right and wrap, the panel sits under them, the card text may wrap; singular forms in the catalogue ("1 day", "1 track"). Verified on a phone viewport against the Vite dev server proxied to the dev API. Tim chose all of them. Built and verified on the local Vite build against the dev API at 390 px (page widths measured, screenshots read, the full sweep on project Smart Parks with the two new track routes clean): cards with `min-w-0` (the entity and device pages were up to 958 px wide from tables inside cards), wrapping repair buttons, the map panning a selected entity above its panel on a phone, devices columns per width (`defaultHiddenSmall`), gateways on by default only with Technical details and their labels from zoom 9, an icon-only base map picker, the attribution folded after load, short traffic times, the explorer's filters behind a button, two-column attention tiles, the keyboard hint hidden, singular catalogue forms, `SWEEP_PROJECT` and stateful map routes in the sweep.

### 2026-09-07, profile pictures (Claude)
...
### 2026-09-07, phase 16 part A built: the device layer (Claude)

- Built in one pass: `GET /projects/{id}/map/devices` (D111) with `tests/api/test_map_devices.py` (an unassigned device with a position, an ended assignment, a device without a position, the viewport bound, a stranger's 403); `ensureDeviceLayers`, `bindDeviceClicks`, `setDevices` and the dashed `track-line-device` layer with `kind` on the track features (D112, D114); `shown_devices` as a shown set with `isDeviceShown`, `toggleDevice`, `showDevices`, `hideAllDevices` and tests (D113); the Devices tab in the layers panel (search, sort, Without an entity first, Tracking an entity after, track toggle, locate, Show all, Hide all, the shown count); on the map page the devices query patched by `position.created`, `?device=` (never with `?entity=`), `?device_tracks=`, device tracks on the card's length with `project_since` for the assigned option, the device panel, the badge counting shown devices, the phone pan for a selected device; the devices list's Map link and the device page's Show on map for a device without an entity; docs and changelog.
- Verified: ruff, mypy, frontend lint, types, catalogue, 34 tests, build. The API test and the exit criteria (an onboarded KPN collar without an entity as a square, SP051307's dashed device track, a late device staying off) wait for CI and the dev server. Not committed.

### 2026-09-07, phase 16 part A live (Claude)

- Committed as 19677ec on Tim's word, pushed, dev server updated. Live check on project Smart Parks: the devices endpoint listed the three tracked collars; a real KPN collar (SP051479, DevEUI 0016C001F016D050) onboarded from Needs attention into the project without an entity appeared under "Without an entity" with its position, off by default, drawn as a square once switched on, with its panel from the marker; SP051307's device track over 30 days is one dashed line next to Tim's solid entity track; the Tracks card counts both. Part A's boxes ticked.
- Found on the way: SP051479 reports a device time in 2030 (its clock is four years ahead), so its last seen shows a negative age; the decoder keeps the device time as it should, and a clock-ahead check is a candidate for the next correctness item.

### 2026-09-07, phase 16 part B built: the all-projects scope (Claude)

- Built in one pass. API: `ScopeContext`, `get_scope_context` and `require_scope_permission` in `deps.py` (the reserved id `all` for a server admin, 403 otherwise, a project id as before); the supported reads switched to it and filter with `context.where` (map current state, tiles, devices and events, tracks, entities, entity assignments, features, groups, events, alerts, gateways, connectivity, coverage, traffic), the event and alert helpers with `all_projects` so the system scope stays apart, `project_id` on map features and device reads, the WebSocket with `all` under one key, the access matrix walking the scope (`ALL_SCOPE_READS`), ADR 0022. Frontend: `lib/scope.ts`, `allScope` on the navigation items with `sectionsFor`, the switcher's All projects entry for server admins, the layout guard that sends other pages opened with `all` to the map, the palette following the scope, the map's panels naming the project and linking into it, the lists with a Project column and links into the object's project, the devices list over every device in the scope, the layers panel with the project as the top level of groups and devices (`projectLayerOf`, `ungroupedLayerOf`, `withProjectShown`, tests).
- Verified: ruff, mypy, frontend lint, types, catalogue, 37 tests, build, OpenAPI regenerated. The scope's API tests and the exit criteria (Smart Parks, Demo park and the benchmark parks on one map with tiles, hiding a park, opening an animal into its project, a live KPN uplink, a project admin's 403) wait for CI and the dev server. Not committed.

### 2026-09-07, phase 16 part B live, and device clocks ahead (Claude and Tim)
...
- Committed as a460276 on Tim's word and deployed; the live call showed the bound was not enough: the all scope over a week took 21 s, a bounding-box scan over every positions chunk of the window (the trap the coverage query fell into). Rewritten per device (3564675), which still took 12 s over the 2,082 devices the dev server has seen in a week, then bounded by devices as well (1616e26): at most the 200 most recently seen devices that qualify, no count pass, `capped`, `devices_scanned` and `devices_total` in the answer and on the card. Live after that: Smart Parks 30 days 0.8 s, the all scope over a week 1.0 s, a benchmark park at the cap 2.7 s.

### 2026-09-09, the heatmap per entity and device (Claude and Tim)

- Tim, testing part c live: the heatmap must work like tracks, one toggle per entity and device, not one global picture. Reworked: `?heat=` and `?device_heat=` list the objects, a heatmap button sits next to the track button on every row of the layers panel and in the entity and device panels, the strip's Heatmaps button carries the count and folds the card, the card counts heatmaps and points, the settings panel lost its scope choice; one request carries both lists. Docs and changelog follow the new shape.

### 2026-09-09, phase 19 part d: drawing, measuring and proximity (Claude)
...
### 2026-09-09, phase 20: the records read, view and export (Claude and Tim)

- Five decisions asked and taken (D142 to D146): one row per moment paged on a time and device cursor with a count, pages with progress and never a refusal, a records dataset with an automatic job, the Records tab first with the Analysis tab kept, small charts per column and a map.
- Built: `shared/records.py` (the moments as a union of position and measurement times, the fill per page over the page's span, the wide header's keys) shared by the read and the export; the records read and count with the access matrix and a test; the `records` export dataset wide and long with a test; the frontend records view with the sequential loader, the virtualized table (`@tanstack/react-virtual` 3.14.11, MIT, pinned), the sparklines, the mini map, the column picker per user, the Export button and "Analyse these"; the explorer's two tabs with the mode in the URL and in saved views; the export dialog's device picker, layout and the job it starts by itself with inline progress.
- Verified: ruff, mypy, frontend lint, types, catalogue, unit tests, the build, OpenAPI regenerated; the API tests run in CI. Browser check with the read stubbed from live positions: 226 rows through the progress bar, eleven columns with units discovered from the rows, the export dialog on the Records dataset. Tim then asked for the first view without the charts and the map: they sit behind a "Charts and map" toggle that starts off (D146 amended). Not committed.
- Committed as 7d0d570 on Tim's word; CI found two things the local checks cannot (Docker is off here): the union's time column was labelled `t`, an attribute of SQLAlchemy's Row, so the page's times came back as rows; and `export_jobs` has a check constraint on the dataset, so the records dataset needs migration 0023. Both fixed in the next commit.
- Deployed 4c77e70 and called the read on the dev server: the count timed out at the proxy. The project filter was a column expression on `positions.project_id` reused for the measurements select, so that half of the union was a cross join of the two hypertables (177 million measurements times the project's positions). Small test data never shows it, since the union removes the duplicates. The filter is now a function of a model's project column, called per table; `tests/shared/test_records.py` compiles the union and checks each half reads one table. The corrected count of a collar's month answers in 1.5 s on the dev server.
