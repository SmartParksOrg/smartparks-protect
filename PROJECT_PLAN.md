...
| D98 | Groups | Folder-like groups per project: `entity_groups` with an optional parent (two levels), an entity in at most one group, devices grouped through their entity; the lists filter by group and the map shows and hides per group | Simple to explain and to render as layers; tags stay possible later for ad hoc filtering. ADR 0020 when built. Decided by Tim on 2026-09-06. |
...
- [ ] Groups (D98, ADR 0020, migration 0016): `entity_groups` per project (name, optional parent, order, colour, icon), `entities.group_id`; API and a Groups page under the project's admin section; group column and filter in the entities and devices lists; bulk onboarding can put new entities in a group.
