...
- Streams are trimmed to about `BUS_MAXLEN` entries (D33). Dead-letter streams are listed, retried and resolved through `/api/v1/attention/dead-letters`. Unknown identities become devices one at a time (`/attention/identities/{id}/create-device`) or many at once (`/attention/identities/bulk-create-devices`, D96: one device type, an optional project and entity type, names from the identity's `name` attribute, which the KPN adapter fills from ThingPark's device name, else the external id; `bulk-ignore` for the rest); the shared `DataTable` has an optional `selection` column for such pages.
