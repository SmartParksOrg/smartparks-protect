...
`services/decoder/protect_decoder/pipeline.py`: driver from the device type, decode, canonical key per record, repeat deliveries link to the existing row in `source_deliveries`, attribution at the record's time, canonical rows and current state in one transaction, domain events after commit. Unknown metric keys are registered automatically in the metric registry with category `uncategorized`. The decoder keeps on `device_current_state` the newest value per metric (`latest_measurements`, key to value and time), the merged `latest_state` with `latest_state_time`, `last_seen_at` from any record, and writes the firmware version a status reports on the device row (migration 0016, D104); `shared/domain/health.py` derives `DeviceHealth` from that and the driver's `health` fields for the device reads (`with_state` in `routers/devices.py`) and the map's current-state features. A LoRaWAN uplink without application payload (port 0, a MAC-only frame) and a port the driver does not know are not failures: the pipeline and the driver put a note on `DecodedRecords.notes`, the trace step shows it, the source event stays `processed` and the connectivity and current state are updated as for any uplink. See `docs/architecture/processing-pipeline.md`.
