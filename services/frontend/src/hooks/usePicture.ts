...
export function usePicture(path: string | null, updatedAt: string | null | undefined): string | null {
  const version = pictureVersion(updatedAt);
  const enabled = Boolean(path && version);
  const query = useQuery({
    queryKey: ["picture", path, version],
    enabled,
    staleTime: Infinity,
    queryFn: async () => {
      ...
      if (known && known.version !== version) URL.revokeObjectURL(known.url);
      const url = URL.createObjectURL(blob);
      urls.set(path as string, { version: version as string, url });
      return url;
    },
  });
