  ...
  return (
    <div className="space-y-2">
      ...
      <div className="flex items-center justify-between text-xs text-muted-foreground">
        ...
        <Button type="button" variant="ghost" size="sm" onClick={() => setPoints([])}>Clear</Button>
      </div>
    </div>
  );
}
...
export function FeaturesPage() {
  ...
  const remove = useMutationToast({ mutationFn: (id: string) => api.delete(`/api/v1/projects/${projectId}/features/${id}`), invalidate: [queryKeys.features(projectId)], success: "Feature deleted", onSuccess: () => setRemoving(null) });
  const columns: ColumnDef<Feature, unknown>[] = [
    { header: "Name", accessorKey: "name" },
    { header: "Type", accessorKey: "feature_type" },
    { header: "Geometry", accessorFn: (f) => f.geometry?.type ?? "" },
    { id: "actions", header: "", cell: ({ row }) => <Button variant="ghost" size="icon" aria-label="Delete feature" onClick={() => setRemoving(row.original)}><Trash2 className="size-4" /></Button> },
  ];
  return (
    <>
      <PageHeader title="Features" description="Sites, zones, geofences and routes drawn on the map" actions={<Button onClick={() => setOpen(true)}><Plus className="size-4" /> New feature</Button>} />
      <Page><DataTable columns={columns} data={features.data?.items} isLoading={features.isPending} emptyMessage="No features yet." /></Page>
      <Dialog open={open} onOpenChange={setOpen}>
        <DialogContent className="sm:max-w-2xl">
          <DialogHeader><DialogTitle>New feature</DialogTitle><DialogDescription>Draw the geometry on the map</DialogDescription></DialogHeader>
          <form className="space-y-4" onSubmit={form.handleSubmit((v) => { if (!geometry) { form.setError("root", { message: "Draw the geometry first" }); return; } create.mutate(v); })} noValidate>
            <div className="grid gap-3 sm:grid-cols-2">
              <Field label="Name" htmlFor="feature-name" error={form.formState.errors.name?.message}><Input id="feature-name" {...form.register("name")} /></Field>
              <Field label="Type" htmlFor="feature-type">
                <Select value={form.watch("feature_type")} onValueChange={(v) => form.setValue("feature_type", v as Values["feature_type"])}>
                  ...
                  <SelectContent><SelectItem value="geofence">geofence</SelectItem><SelectItem value="zone">zone</SelectItem><SelectItem value="site">site</SelectItem><SelectItem value="route">route</SelectItem></SelectContent>
                </Select>
              </Field>
            </div>
            ...
            <DialogFooter><Button type="button" variant="outline" onClick={() => setOpen(false)}>Cancel</Button><Button type="submit" disabled={create.isPending}>Save</Button></DialogFooter>
          </form>
        </DialogContent>
      </Dialog>
      <ConfirmDialog open={removing != null} onOpenChange={(o) => !o && setRemoving(null)} title="Delete feature" description={`${removing?.name} is removed from the map.`} confirmLabel="Delete" onConfirm={() => removing && remove.mutate(removing.id)} pending={remove.isPending} />
    </>
  );
}
