...
export function ExplorerPage() {
  ...
  const columns: ColumnDef<LongRow, unknown>[] = [
    { header: "Time", accessorKey: "time", cell: ({ getValue }) => formatInZone(getValue<string>(), state.timezone) },
    { header: "Metric", accessorKey: "metric_key", cell: ({ getValue }) => metricLabels.get(getValue<string>()) ?? getValue<string>() },
    { header: "Entity", accessorKey: "owner_name" },
  ...
  return (
    <>
      <PageHeader
        title="Data explorer"
        description="Server side aggregates of the project's measurements; drill down to the rows and source events behind a bucket"
        actions={<>
          <Select value={currentView?.id ?? "none"} onValueChange={(id) => { const v = views.data?.items.find((x) => x.id === id); if (v) applyView(v); }}>
            <SelectTrigger className="w-48" aria-label="Saved views"><SelectValue placeholder="Saved views" /></SelectTrigger>
            <SelectContent>
          ...
          <Button variant="outline" onClick={() => setSaveOpen(true)} disabled={state.metrics.length === 0}><Bookmark className="size-4" /> Save view</Button>
          {currentView && (currentView.created_by === user?.id || canAdmin(role)) && (
            <Button variant="outline" size="icon" aria-label="Delete saved view" onClick={() => deleteView.mutate(currentView.id)}><Trash2 className="size-4" /></Button>
          )}
          <Button onClick={() => setExportOpen(true)} disabled={state.metrics.length === 0}><Download className="size-4" /> Export</Button>
        </>}
      />
      <Page>
        <div className="grid gap-3 md:grid-cols-3 xl:grid-cols-6">
          <Field label="Metrics" htmlFor="metrics">
            <MultiSelect options={(metrics.data ?? []).map((m) => ({ value: m.key, label: m.label, hint: m.unit ?? undefined }))} value={state.metrics} onChange={(v) => update({ metrics: v })} placeholder="Choose metrics" label="metrics" className="w-full" maxSelected={20} />
          </Field>
          <Field label="Entities" htmlFor="entities" hint={state.entities.length === 0 ? "All entities" : undefined}>
            <MultiSelect options={(entities.data?.items ?? []).map((e) => ({ value: e.id, label: e.name }))} value={state.entities} onChange={(v) => update({ entities: v })} placeholder="All entities" label="entities" className="w-full" maxSelected={20} />
          </Field>
          ...
          <Field label="Bucket" htmlFor="bucket" hint={series.data ? `${series.data.automatic_bucket ? "automatic, " : ""}${bucketLabel(series.data.bucket_seconds)}` : undefined}>
          ...
          <Field label="Aggregates" htmlFor="aggregates" hint={`Chart shows ${primary}`}>
            <MultiSelect options={AGGREGATES.map((a) => ({ value: a, label: a }))} value={state.aggregates} onChange={(v) => update({ aggregates: v.length ? (v as Aggregate[]) : ["mean"] })} placeholder="Aggregates" label="aggregates" className="w-full" />
          </Field>
        ...
        {state.metrics.length === 0 ? (
          <EmptyState icon={ChartLine} title="Choose a metric to start" description="Metrics with data in the last year are listed. Pick entities to compare, or leave the field empty for the whole project." />
        ) : (
          <>
            ...
            <DataTable columns={columns} data={rows} isLoading={series.isPending} emptyMessage="No measurements in this range." onRowClick={setDrill} footer={series.data && `${rows.length} rows, ${series.data.series?.length ?? 0} series, bucket ${bucketLabel(series.data.bucket_seconds)}`} />
          </>
        )}
      </Page>
    ...
    <>
      <Dialog open={row !== null} onOpenChange={(o) => !o && onClose()}>
        <DialogContent className="max-h-[85vh] overflow-y-auto sm:max-w-3xl">
          <DialogHeader>
            <DialogTitle>Rows behind the bucket</DialogTitle>
          ...
          <DataTable columns={columns} data={rows.data?.items} isLoading={rows.isPending} emptyMessage="No rows." footer={rows.data?.next_cursor ? "Showing the first 500 rows" : undefined} />
        </DialogContent>
      </Dialog>
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-md">
        <DialogHeader><DialogTitle>Save this view</DialogTitle><DialogDescription>Metrics, entities, range, bucket, aggregates and chart, shared with the project.</DialogDescription></DialogHeader>
        <Field label="Name" htmlFor="view-name"><Input id="view-name" value={name} onChange={(e) => setName(e.target.value)} placeholder="Battery, last 30 days" /></Field>
        <DialogFooter><Button onClick={() => onSave(name.trim())} disabled={!name.trim() || pending}>Save</Button></DialogFooter>
      </DialogContent>
    </Dialog>
  );
}
