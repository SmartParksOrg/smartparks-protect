...
import {
  ...
  MapTile,
  SavedViewTile,
} from "@/components/dashboards/tiles";
...
export function DashboardsPage() {
  ...
  const dashboards = useQuery({
    ...
    queryFn: () =>
      api.get<PageType<Dashboard>>(base, { query: { limit: 100 } }),
  });
  ...
  const selectedId =
    params.get("dashboard") ?? dashboards.data?.items[0]?.id ?? null;
  const current =
    dashboards.data?.items.find((d) => d.id === selectedId) ?? null;
  ...
  const create = useMutationToast({
    mutationFn: (name: string) =>
      api.post<Dashboard>(base, {
        body: {
          name,
          tiles: [
            { id: "map", kind: "map", size: "l" },
            { id: "alerts", kind: "alerts", size: "m" },
            { id: "events", kind: "events", size: "m" },
          ],
        },
      }),
  ...
  const save = useMutationToast({
    mutationFn: () =>
      api.patch<Dashboard>(`${base}/${current?.id}`, { body: { tiles } }),
    invalidate: [queryKeys.dashboards(projectId)],
  ...
  const remove = useMutationToast({
    mutationFn: () => api.delete<void>(`${base}/${current?.id}`),
    invalidate: [queryKeys.dashboards(projectId)],
    success: t("Dashboard deleted"),
    onSuccess: () => {
      setDeleting(false);
      setParams(
        (p) => {
  ...
  const startEdit = () => {
    setDraft((current?.tiles ?? []) as Tile[]);
    setEditing(true);
  };
  ...
  const update = (index: number, patch: Partial<Tile>) =>
    setDraft(tiles.map((t, i) => (i === index ? { ...t, ...patch } : t)));
  ...
  return (
    <>
      <PageHeader
        title={t("Dashboards")}
        description={t(
          "Saved views and live tiles on a grid, shared with the project",
        )}
        actions={
          <>
            {dashboards.data && dashboards.data.items.length > 0 && (
              <Select value={selectedId ?? ""} onValueChange={select}>
                <SelectTrigger className="w-56" aria-label={t("Dashboard")}>
                  <SelectValue placeholder={t("Choose a dashboard")} />
                </SelectTrigger>
                <SelectContent>
            ...
            {admin && editing && (
              <>
                <Button variant="outline" onClick={() => setAdding(true)}>
                  <Plus className="size-4" /> {t("Add tile")}
                </Button>
                <Button
                  variant="ghost"
                  onClick={() => {
            ...
            {admin && (
              <Button onClick={() => setCreating(true)}>
                <Plus className="size-4" /> {t("New dashboard")}
              </Button>
            )}
          </>
        }
      ...
      <Page>
        {dashboards.error && (
          <Callout kind="error">{dashboards.error.message}</Callout>
        )}
        {dashboards.isSuccess && dashboards.data.items.length === 0 && (
          <EmptyState
          ...
          <div className="grid grid-cols-12 gap-4">
            {tiles.map((tile, index) => (
              <Card key={tile.id} className={SIZES[tile.size] ?? SIZES.m}>
                  ...
                  {editing && (
                    <span className="flex items-center gap-1">
                      <Select
                        value={tile.size}
                        onValueChange={(v) =>
                          update(index, { size: v as Tile["size"] })
                        }
                      >
                        <SelectTrigger
                          className="h-7 w-24"
                        ...
                        <SelectContent>
                          <SelectItem value="s">{t("small")}</SelectItem>
                          <SelectItem value="m">{t("medium")}</SelectItem>
                          <SelectItem value="l">{t("large")}</SelectItem>
                        </SelectContent>
                      </Select>
                      <Button
                        size="icon"
                      ...
                      <Button
                        ...
                        onClick={() =>
                          setDraft(tiles.filter((_, i) => i !== index))
                        }
                      >
                        <X className="size-4" />
                      </Button>
                ...
                <CardContent className="h-72 overflow-auto">
                  {tile.kind === "saved_view" &&
                    (views.data?.items.find(
                      (v) => v.id === tile.saved_view_id,
                    ) ? (
                      <SavedViewTile
                        projectId={projectId}
                        view={views.data.items.find(
                          (v) => v.id === tile.saved_view_id,
                        )!}
                      />
                    ) : (
                      <p className="text-sm text-muted-foreground">
                        {t("The saved view no longer exists.")}
                      </p>
                    ))}
                  {tile.kind === "map" && <MapTile projectId={projectId} />}
                  {tile.kind === "alerts" && (
                    <AlertsTile projectId={projectId} />
                  )}
                  {tile.kind === "events" && (
                    <EventsTile projectId={projectId} />
                  )}
            ...
            {tiles.length === 0 && (
              <div className="col-span-12 text-sm text-muted-foreground">
                {t("This dashboard has no tiles yet.")}
                {admin && !editing ? " Edit it to add some." : ""}
              </div>
            )}
          </div>
        )}
        {current && admin && !editing && (
          <div className="text-right">
            <Button variant="ghost" size="sm" onClick={() => setDeleting(true)}>
              <Trash2 className="size-4" /> {t("Delete dashboard")}
            </Button>
          </div>
        )}
      </Page>
      <Dialog open={creating} onOpenChange={setCreating}>
        <DialogContent className="sm:max-w-md">
          <DialogHeader>
            ...
            <DialogDescription>
              {t(
                "It starts with the map, open alerts and recent events; edit it afterwards.",
              )}
            </DialogDescription>
          </DialogHeader>
          <Field label={t("Name")} htmlFor="dashboard-name">
            <Input
              id="dashboard-name"
              value={newName}
          ...
          <div className="space-y-2">
            ...
            {(views.data?.items ?? []).map((v) => (
              <Button
                ...
                variant="outline"
                className="w-full justify-start"
                onClick={() => addTile("saved_view", v.id)}
              >
                {t("Chart: {{name}}", { name: v.name })}
              </Button>
            ))}
            {views.data?.items.length === 0 && (
              <p className="text-xs text-muted-foreground">
                {t("Save a view in the Data Explorer to add charts.")}
              </p>
            )}
          </div>
        </DialogContent>
      ...
      <ConfirmDialog
        ...
        description={t("Tiles are only layout; saved views and data stay.")}
