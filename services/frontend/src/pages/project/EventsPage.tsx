...
export function AlertActions({
  ...
  const act = useMutationToast({
    mutationFn: ({ action }: { action: "acknowledge" | "resolve" }) =>
      api.post<Alert>(`${scopeBase(scope)}/alerts/${alert.id}/${action}`, {
        body: { note: note || null },
      }),
    success: (a) => `Alert ${a.status}`,
    onSuccess: () => {
      void client.invalidateQueries({ queryKey: ["alerts", scope] });
      void client.invalidateQueries({ queryKey: ["events", scope] });
      onDone?.();
    },
  });
  if (alert.status === "resolved")
    return (
      <div className="text-sm text-muted-foreground">
        {t("Resolved")} {formatTime(alert.resolved_at)}
...
export function EventDetailDialog({
  ...
  const detail = useQuery({
    queryKey: queryKeys.event(scope, eventId ?? ""),
    queryFn: () =>
      api.get<EventDetail>(`${scopeBase(scope)}/events/${eventId}`),
      ...
      <DialogContent className="max-h-[85vh] overflow-y-auto sm:max-w-2xl">
        <DialogHeader>
          <DialogTitle className="flex items-center gap-2">
            {d && (
              <Icon
                iconKey={eventIcon(d.event.event_type)}
          ...
          <div className="space-y-4 text-sm">
            <div className="grid grid-cols-2 gap-2">
              <div>
                ...
                <div>
                  <StatusBadge value={d.event.severity} />
                </div>
              </div>
              <div>
                <span className="text-muted-foreground">{t("Created")}</span>
                <div>{formatTime(d.event.created_at)}</div>
              </div>
              {d.event.entity_id && projectPath && (
                <div>
                  <span className="text-muted-foreground">{t("Entity")}</span>
                  <div>
                    <Link
                      className="underline"
                      to={`/projects/${projectFor(scope, d.event.project_id)}/entities/${d.event.entity_id}`}
                    >
                      {d.event.entity_name ?? t("open entity")}
                    </Link>{" "}
                    ·{" "}
                    <Link
                      className="underline"
                      to={`/projects/${projectFor(scope, d.event.project_id)}/map?entity=${d.event.entity_id}`}
                    >
                      {t("show on map")}
                    </Link>
                  </div>
              ...
              {d.event.device_id && projectPath && (
                <div>
                  <span className="text-muted-foreground">{t("Device")}</span>
                  <div>
                    <Link
                      className="underline"
                      to={`/projects/${projectFor(scope, d.event.project_id)}/devices/${d.event.device_id}`}
                    >
                      {d.event.device_name ?? t("open device")}
                    </Link>
                  </div>
                </div>
              )}
              {d.event.trace_id && projectPath && (
                <div>
                  <span className="text-muted-foreground">{t("Trace")}</span>
                  <div>
                    <Link
                      className="underline"
                      to={`/projects/${projectFor(scope, d.event.project_id)}/network/traces?trace=${d.event.trace_id}`}
                    >
            ...
            {d.alert && (
              <div className="rounded-md border p-3">
                <div className="mb-2 flex items-center gap-2 font-medium">
                  {t("Alert")} <StatusBadge value={d.alert.status} />
                </div>
                <AlertActions
                  scope={scope}
                  alert={d.alert}
            ...
            <div>
              ...
              {d.deliveries.length === 0 ? (
                <div className="text-muted-foreground">
                  {t("No automation acted on this event.")}
                </div>
              ) : (
                <ul className="space-y-1">
                  {d.deliveries.map((x) => (
                    ...
                    >
                      ...
                      <span>{x.action_type}</span>
                      <span className="text-xs text-muted-foreground">
                        {x.attempts} {t("attempt")}
                        {x.attempts === 1 ? "" : "s"}
                        {x.delivered_at
                          ? `, delivered ${formatTime(x.delivered_at)}`
                          : ""}
                      </span>
                      {x.error_message && (
            ...
            {technicalContext(d.event.context) && (
              <details className="rounded-md border px-3 py-2">
                ...
                <div className="mt-2">
                  <JsonView value={technicalContext(d.event.context)} />
                </div>
              </details>
            )}
          </div>
...
export function EventsPage({ scope: scopeProp }: { scope?: Scope } = {}) {
  ...
  const events = useQuery({
    queryKey: queryKeys.events(scope, query),
    queryFn: () =>
      api.get<PageType<EventItem>>(`${scopeBase(scope)}/events`, { query }),
  });
  const client = useQueryClient();
  useProjectStream(scope === "server" ? undefined : scope, (m) => {
    if (m.topic === "event.created")
      void client.invalidateQueries({ queryKey: ["events", scope] });
  });
      ...
      ? [
          {
            id: "project",
            header: t("Project"),
            accessorFn: (e: EventItem) => projectName(e.project_id),
          } as ColumnDef<EventItem, unknown>,
        ]
      : []),
    {
      header: t("Time"),
    ...
    <>
      ...
      <Page>
        <div className="flex flex-wrap items-end gap-2">
          <Input
            className="w-48"
            placeholder={t("Event type, e.g. GEOFENCE_EXIT")}
            ...
            onChange={(e) => {
              ...
              setParams(
                (p) => {
                  if (e.target.value)
                    p.set("type", e.target.value.toUpperCase());
                  else p.delete("type");
                  return p;
                },
                { replace: true },
          ...
          <Select
            ...
            onValueChange={(v) => {
              ...
              setParams(
                (p) => {
                  if (v === "all") p.delete("severity");
                  else p.set("severity", v);
                  return p;
                },
                { replace: true },
              );
          ...
          >
            <SelectTrigger className="w-36" aria-label={t("Severity")}>
              <SelectValue />
            </SelectTrigger>
            <SelectContent>
              <SelectItem value="all">{t("Any severity")}</SelectItem>
              {SEVERITIES.map((s) => (
        ...
        <DataTable
          ...
          data={items}
          searchable
          isLoading={events.isPending && pages.length === 0}
          emptyMessage={t(
            "No events yet. Enable a rule, or wait for a device event.",
          )}
          onRowClick={(e) =>
            setParams(
              (p) => {
