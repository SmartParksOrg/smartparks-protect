...
import { contactTracingPresentation } from "@/components/analysis/presentations";
...
const MODULE = "contact_tracing";

/** Contact tracing (docs/analytics/contact-tracing.md): the runs of the module as a table,
 * "New analysis" opening the dialog with the form, and an opened run with what it found, the
 * contact network, the pair and subject tables, and the map of where pairs met. */
export function ContactTracingPage() {
  ...
  const presentation = contactTracingPresentation(t, projectId);
  ...
  return (
    <>
      <PageHeader
        title={t("Contact tracing")}
        description={t(
          "Which subjects were near each other, from the sightings devices reported and the proximity their fixes show",
        )}
        actions={
          can("analysis:run") ? (
            <Button
