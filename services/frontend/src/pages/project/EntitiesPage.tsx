...
export function EntitiesPage() {
  ...
  const entities = useQuery({ queryKey: [...queryKeys.entities(projectId), q, group], queryFn: () => api.get<PageType<Entity>>(`/api/v1/projects/${projectId}/entities`, { query: { q: q || undefined, group_id: group && group !== UNGROUPED ? group : undefined, ungrouped: group === UNGROUPED ? true : undefined, limit: 500 } }), placeholderData: (previous) => previous });
  ...
  const columns: ColumnDef<Entity, unknown>[] = [
    ...
    ...(groups.data && groups.data.length > 0 ? [{ header: t("Group"), accessorFn: (e: Entity) => groupName(e.group_id) } as ColumnDef<Entity, unknown>] : []),
  ...
  return (
    <>
      <PageHeader title={t("Entities")} description={t("Animals, people, vehicles, gates and other monitored objects")} actions={<>
        <GroupSelect projectId={projectId} mode="filter" value={group} onChange={setGroup} className="h-9 w-44" />
        {canAdmin(role) && <Button onClick={() => { setEditing(null); setOpen(true); }}><Plus className="size-4" /> {t("New entity")}</Button>}
      </>} />
      <Page>
