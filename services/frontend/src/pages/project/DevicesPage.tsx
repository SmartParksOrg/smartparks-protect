...
export function DevicesPage() {
  ...
  const devices = useQuery({ queryKey: queryKeys.devices({ projectId, q, group }), queryFn: () => api.get<PageType<Device>>("/api/v1/devices", { query: { project_id: projectId, q: q || undefined, group_id: group && group !== UNGROUPED ? group : undefined, limit: 500 } }), placeholderData: (previous) => previous, select: (page) => (group === UNGROUPED ? { ...page, items: page.items.filter((d) => !d.group_id) } : page) });
  const groups = useGroups(projectId);
  const groupName = (id: string | null | undefined) => groups.data?.find((g) => g.id === id)?.name ?? "";
  ...
  const columns: ColumnDef<Device, unknown>[] = [
    ...
    { header: t("Entity"), accessorKey: "entity_name", cell: ({ row }) => (row.original.entity_id ? <Link className="underline" to={`/projects/${projectId}/entities/${row.original.entity_id}`} onClick={(ev) => ev.stopPropagation()}>{row.original.entity_name}</Link> : <span className="text-muted-foreground">{t("none")}</span>) },
    ...(groups.data && groups.data.length > 0 ? [{ header: t("Group"), accessorFn: (d: Device) => groupName(d.group_id) } as ColumnDef<Device, unknown>] : []),
  ...
  return (
    <>
      <PageHeader title={t("Devices")} description={t("Hardware currently assigned to this project")} actions={<GroupSelect projectId={projectId} mode="filter" value={group} onChange={setGroup} className="h-9 w-44" />} />
      <Page>
