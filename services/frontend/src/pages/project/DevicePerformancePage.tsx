...
import { devicePerformancePresentation } from "@/components/analysis/presentations";
...
const MODULE = "device_performance";

/** Device performance (docs/ANALYTICS_DEVICE_PERFORMANCE_PLAN.md): the runs of the module as
 * a table, "New analysis" opening the dialog with the form, and an opened run with its fleet
 * table, the device sections, the map and the area tables; "Analyse performance" on a device
 * page or the devices list opens the dialog with the devices filled in. */
export function DevicePerformancePage() {
  ...
  const presentation = devicePerformancePresentation(t, projectId);
  ...
  return (
    <>
      <PageHeader
        title={t("Device performance")}
        description={t(
          "Health, reporting, GNSS and network of the devices, as a fleet and one by one",
        )}
        actions={
          can("analysis:run") ? (
            <Button
