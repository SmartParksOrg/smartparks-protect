...
export function UsersPage() {
  ...
  const update = useMutationToast({ mutationFn: ({ id, body }: { id: string; body: Record<string, boolean> }) => api.patch(`/api/v1/admin/users/${id}`, { body }), invalidate: [queryKeys.users], success: "User updated" });
  ...
  const revoke = useMutationToast({ mutationFn: (id: string) => api.delete(`/api/v1/admin/invitations/${id}`), invalidate: [queryKeys.serverInvitations], success: "Invitation revoked" });
  const userColumns: ColumnDef<UserAdmin, unknown>[] = [
    { header: "Email", accessorKey: "email" },
    { header: "Name", accessorKey: "full_name" },
    { header: "Active", accessorKey: "is_active", cell: ({ row }) => <Switch checked={row.original.is_active} disabled={row.original.id === me?.id} onCheckedChange={(v) => update.mutate({ id: row.original.id, body: { is_active: v } })} aria-label="Active" /> },
    { header: "Server admin", accessorKey: "is_superuser", cell: ({ row }) => <Switch checked={row.original.is_superuser} disabled={row.original.id === me?.id} onCheckedChange={(v) => update.mutate({ id: row.original.id, body: { is_superuser: v } })} aria-label="Server admin" /> },
    { header: "Last login", accessorKey: "last_login_at", cell: ({ getValue }) => formatAgo(getValue<string | null>()) },
  ...
  const invitationColumns: ColumnDef<Invitation, unknown>[] = [
    { header: "Email", accessorKey: "email" },
    { header: "Kind", accessorFn: (i) => (i.server_admin ? "server admin" : i.role ?? "") },
  ...
  return (
    <>
      <PageHeader title="Users" description="Every account on this server and the open invitations" />
      <Page>
        <Card>
          <CardHeader><CardTitle>Invite a server admin</CardTitle></CardHeader>
          <CardContent>
            <form className="flex flex-wrap items-end gap-3" onSubmit={(e) => { e.preventDefault(); if (email) invite.mutate(); }}>
              <Input type="email" placeholder="email address" className="w-64" value={email} onChange={(e) => setEmail(e.target.value)} aria-label="Email" />
              <Button type="submit" disabled={!email || invite.isPending}>Invite</Button>
            </form>
            {note && <Callout kind="warning" className="mt-3">{note}</Callout>}
            <p className="mt-2 text-xs text-muted-foreground">Project members are invited by project admins under the project's Members page.</p>
          </CardContent>
        </Card>
