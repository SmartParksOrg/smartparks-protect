        ...
        {d && (
          <Tabs defaultValue="identities">
            ...
            <TabsContent value="entity" className="space-y-3">
              <ul className="text-sm">{d.entity_assignments.map((a) => <li key={a.id}>{t("Entity {{id}}: {{from}} to {{to}}", { id: a.entity_name ?? a.entity_id.slice(0, 8), from: formatTime(a.valid_from), to: a.valid_to ? formatTime(a.valid_to) : t("now") })}</li>)}{d.entity_assignments.length === 0 && <li className="text-muted-foreground">{t("Not assigned to an entity.")}</li>}</ul>
