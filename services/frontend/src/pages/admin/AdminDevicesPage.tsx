...
function ManageDevice({
  ...
  const entities = useQuery({
    queryKey: queryKeys.entities(entityAssignment.project_id),
    queryFn: () =>
      api.get<PageType<Entity>>(
        `/api/v1/projects/${entityAssignment.project_id}/entities`,
        { query: { limit: 500 } },
      ),
    enabled: Boolean(entityAssignment.project_id),
  });
  const invalidate = [queryKeys.device(deviceId), queryKeys.devices({})];
  const addIdentity = useMutationToast({
    mutationFn: () =>
      api.post(`/api/v1/devices/${deviceId}/identities`, { body: identity }),
    invalidate,
    success: t("Identity linked"),
    onSuccess: () => setIdentity({ data_source_id: "", external_id: "" }),
  });
  const assign = useMutationToast({
    mutationFn: () =>
      api.post(`/api/v1/devices/${deviceId}/project-assignments`, {
        body: {
  ...
  const assignEntity = useMutationToast({
    mutationFn: () =>
      api.post(
        `/api/v1/projects/${entityAssignment.project_id}/entity-assignments`,
        {
          body: {
            device_id: deviceId,
            entity_id: entityAssignment.entity_id,
            valid_from: entityAssignment.valid_from,
          },
        },
      ),
    invalidate: [
      ...invalidate,
      queryKeys.currentState(entityAssignment.project_id),
    ],
    success: t("Assigned to entity"),
  });
  const d = detail.data;
      ...
      <DialogContent className="max-h-[85vh] overflow-y-auto sm:max-w-2xl">
        ...
        {d && (
          <Tabs defaultValue="identities">
            <TabsList>
              <TabsTrigger value="identities">{t("Identities")}</TabsTrigger>
            ...
            <TabsContent value="identities" className="space-y-3">
              <ul className="text-sm">
                ...
                {d.external_identities.length === 0 && (
                  <li className="text-muted-foreground">{t("None yet.")}</li>
                )}
              </ul>
              <div className="grid gap-2 sm:grid-cols-[1fr_1fr_auto]">
                <Select
                  value={identity.data_source_id}
                  onValueChange={(v) =>
                    setIdentity({ ...identity, data_source_id: v })
                  }
                >
                  <SelectTrigger>
                ...
                <Input
                  placeholder={t("DevEUI or external id")}
                  className="font-mono"
                  value={identity.external_id}
                  onChange={(e) =>
                    setIdentity({
                      ...identity,
                      external_id: e.target.value.trim().toUpperCase(),
                    })
                  }
                />
                <Button
              ...
              <ul className="text-sm">
                {d.project_assignments.map((a) => (
                  <li key={a.id}>
                    {t("{{name}}: {{from}} to {{to}}", {
                      name:
                        projects.data?.items.find((p) => p.id === a.project_id)
                          ?.name ?? a.project_id.slice(0, 8),
                ...
                <Card>
                  ...
                  <CardContent className="grid gap-2 sm:grid-cols-3">
                    <Select
                      ...
                      onValueChange={(v) =>
                        setAssignment({ ...assignment, project_id: v })
                      }
                    >
                      <SelectTrigger>
                    ...
                    <AssignmentStartField
                      ...
                      onChange={(iso) =>
                        setAssignment((a) => ({ ...a, valid_from: iso }))
                      }
                    />
                    <Button
                      disabled={!assignment.project_id || assign.isPending}
            ...
            <TabsContent value="entity" className="space-y-3">
              <ul className="text-sm">
                {d.entity_assignments.map((a) => (
                  <li key={a.id}>
                    {t("Entity {{id}}: {{from}} to {{to}}", {
                      id: a.entity_name ?? a.entity_id.slice(0, 8),
                      from: formatTime(a.valid_from),
                      to: a.valid_to ? formatTime(a.valid_to) : t("now"),
                    })}
                  </li>
              ...
              <div className="grid gap-2 sm:grid-cols-2">
                <Select
                  value={entityAssignment.project_id}
                  onValueChange={(v) =>
                    setEntityAssignment({
                      ...entityAssignment,
                      project_id: v,
                    ...
                    {d.project_assignments
                      ...
                      .map((pid) => (
                        <SelectItem key={pid} value={pid}>
                          {projects.data?.items.find((p) => p.id === pid)
                            ?.name ?? pid}
                        </SelectItem>
                      ))}
                  </SelectContent>
                ...
                <Select
                  value={entityAssignment.entity_id}
                  onValueChange={(v) =>
                    setEntityAssignment({ ...entityAssignment, entity_id: v })
                  }
                  disabled={!entityAssignment.project_id}
                >
                  <SelectTrigger>
                ...
                <AssignmentStartField
                  ...
                  joinedAt={
                    d?.project_assignments.find(
                      (a) => a.project_id === entityAssignment.project_id,
                    )?.valid_from ?? null
                  }
                  timeZone={
                    projects.data?.items.find(
                      (p) => p.id === entityAssignment.project_id,
                    )?.timezone ?? "UTC"
                  }
                  onChange={(iso) =>
                    setEntityAssignment((a) => ({ ...a, valid_from: iso }))
                  }
                />
                <Button
                  disabled={
                    !entityAssignment.entity_id || assignEntity.isPending
                  }
                  onClick={() => assignEntity.mutate()}
                >
                  {t("Assign to entity")}
                </Button>
                <p className="text-xs text-muted-foreground sm:col-span-2">
                  {t(
                    "The device must belong to the project at the start time. An open assignment of this device to another entity is rejected; end it first.",
                  )}
                </p>
              </div>
            </TabsContent>
...
export function AdminDevicesPage() {
  ...
  // The filters live in the URL so a filtered list is a link; the search box stays local.
  ...
  const devices = usePages<Device>({
    queryKey: queryKeys.devices({ q, source, project, type, status }),
    fetchPage: (cursor) =>
      api.get<PageType<Device>>("/api/v1/devices", {
        query: {
          q: q || undefined,
          data_source_id: source || undefined,
          project_id: project && project !== "none" ? project : undefined,
          in_no_project: project === "none" ? true : undefined,
          device_type_id: type || undefined,
  ...
  const projectName = (id: string | null | undefined) =>
    projects.data?.items.find((p) => p.id === id)?.name ?? "";
  ...
  const create = useMutationToast({
    mutationFn: (values: DeviceValues) =>
      api.post<Device>("/api/v1/devices", {
        body: {
          ...values,
  ...
  const importCsv = useMutationToast({
    mutationFn: (file: File) => {
      ...
      return api.post<{ created: number }>("/api/v1/devices/import", { body });
    },
    invalidate: [queryKeys.devices({})],
  ...
  const columns: ColumnDef<Device, unknown>[] = [
    ...
    },
    {
      id: "source",
      header: t("Source"),
      accessorFn: (d) => (d.data_source_names ?? []).join(", "),
      cell: ({ row }) =>
        (row.original.data_source_names ?? []).length > 0 ? (
          (row.original.data_source_names ?? []).join(", ")
        ) : (
    ...
    },
    {
      id: "project",
      header: t("Project"),
      accessorFn: (d) => projectName(d.project_id) || t("none"),
      cell: ({ row }) =>
    ...
    },
    {
      id: "entity",
      header: t("Entity"),
      accessorFn: (d) => d.entity_name ?? "",
      cell: ({ row }) =>
        row.original.entity_id && row.original.project_id ? (
          <Link
            className="underline"
            to={`/projects/${row.original.project_id}/entities/${row.original.entity_id}`}
    ...
    { id: "serial", header: t("Serial"), accessorKey: "serial_number" },
    {
      id: "last_seen",
      header: t("Last seen"),
      meta: { filter: false },
      ...
      cell: ({ row }) => (
        <div className="flex gap-1">
          <Button
            ...
            onClick={(e) => {
              e.stopPropagation();
              setParams({ device: row.original.id });
            }}
          >
            {t("Manage")}
          </Button>
          <Button
            ...
            onClick={(e) => {
              e.stopPropagation();
              void navigate(`/admin/devices/${row.original.id}`);
            }}
          >
            {t("Details")}
          </Button>
  ...
  const filtersSet = [source, project, type, status].filter(Boolean).length;
  return (
    <>
      <PageHeader
        title={t("Devices")}
        description={t("Every device on the server, whichever project owns it")}
        actions={
          <>
            ...
            <Button onClick={() => setOpen(true)}>
              <Plus className="size-4" /> {t("New device")}
            </Button>
          </>
        }
      />
      <Page>
        <div className="flex flex-wrap items-center gap-2">
          ...
          >
            <SelectTrigger className="h-9 w-48" aria-label={t("Source")}>
              <SelectValue />
            </SelectTrigger>
            <SelectContent>
              <SelectItem value="all">{t("Any source")}</SelectItem>
              {(sources.data?.items ?? []).map((s) => (
                <SelectItem key={s.id} value={s.id}>
                  {s.name}
                </SelectItem>
          ...
          <Select
            value={project || "all"}
            onValueChange={(v) => setFilter("project", v === "all" ? "" : v)}
          >
            <SelectTrigger className="h-9 w-48" aria-label={t("Project")}>
              <SelectValue />
            </SelectTrigger>
            <SelectContent>
          ...
          >
            <SelectTrigger className="h-9 w-44" aria-label={t("Type")}>
              <SelectValue />
            </SelectTrigger>
            <SelectContent>
              <SelectItem value="all">{t("Any type")}</SelectItem>
          ...
          >
            <SelectTrigger className="h-9 w-36" aria-label={t("Status")}>
              <SelectValue />
            </SelectTrigger>
            <SelectContent>
              <SelectItem value="all">{t("Any status")}</SelectItem>
              {["active", "inventory", "repair", "retired"].map((s) => (
          ...
          {filtersSet > 0 && (
            <Button
              size="sm"
              variant="ghost"
              onClick={() => setParams(managing ? { device: managing } : {})}
            >
              {t("Clear filters")}
            </Button>
          )}
        ...
        <Callout kind="info">
          {t(
            "CSV columns: device_name, external_identifier, device_type, datasource, project, effective_from (ISO 8601 with offset), entity (optional). All rows or none.",
          )}
        </Callout>
        <DataTable
          columns={columns}
          columnsKey="admin-devices"
          defaultHidden={["driver", "status", "serial", "created"]}
          defaultHiddenSmall={["source", "project", "type"]}
          columnFilters
          data={devices.items}
          searchable
          onSearchChange={setQ}
          ...
          onRowClick={(d) => setParams({ device: d.id })}
        />
      </Page>
      <Dialog open={open} onOpenChange={setOpen}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>{t("New device")}</DialogTitle>
          </DialogHeader>
          <form
            className="space-y-3"
          ...
          >
            ...
            <Field
              label={t("Type")}
              htmlFor="d-type"
              error={form.formState.errors.device_type_id?.message}
            >
              <Select
                value={form.watch("device_type_id")}
                onValueChange={(v) =>
                  form.setValue("device_type_id", v, { shouldValidate: true })
                }
              >
            ...
            <div className="grid gap-3 sm:grid-cols-2">
              <Field label={t("Serial number")} htmlFor="d-serial">
                <Input id="d-serial" {...form.register("serial_number")} />
              </Field>
              <Field label={t("Status")} htmlFor="d-status">
                <Select
                  value={form.watch("status")}
                  onValueChange={(v) =>
                    form.setValue("status", v as DeviceValues["status"])
                  }
                >
      ...
      {managing && (
        <ManageDevice deviceId={managing} onClose={() => setParams({})} />
      )}
    </>
  );
}
