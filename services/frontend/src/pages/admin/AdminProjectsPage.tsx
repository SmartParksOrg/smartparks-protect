...
export function AdminProjectsPage() {
    ...
    mutationFn: (values: Values) =>
      api.post<Project>("/api/v1/projects", {
        body: {
          ...values,
          organization_id:
            values.organization_id === NONE ? null : values.organization_id,
        },
      }),
    invalidate: [queryKeys.projects],
  ...
  const columns: ColumnDef<ProjectWithRole, unknown>[] = [
    { header: t("Name"), accessorKey: "name" },
    {
      ...
      accessorFn: (p) =>
        organizationById.get(p.organization_id ?? "")?.name ?? "",
    },
      ...
      <Page>
        <div className="flex items-center gap-2">
          <Select value={filter} onValueChange={setFilter}>
            <SelectTrigger
              className="w-64"
              aria-label={t("Filter by organization")}
            >
              <SelectValue />
            </SelectTrigger>
            <SelectContent>
        ...
        <DialogContent>
          ...
          >
            ...
            <Field
              ...
              hint={t("Short identifier used in URLs and exports")}
            >
            ...
            >
              <TimezoneSelect
                ...
                first={[
                  ...new Set(
                    (projects.data?.items ?? []).map((p) => p.timezone),
                  ),
                ]}
              />
            </Field>
            <Field
              ...
              htmlFor="p-org"
              hint={t(
                "A grouping for the admin pages, not a permission boundary",
              )}
            >
              <Select
...
function OrganizationsCard({
  ...
  const create = useMutationToast({
    mutationFn: (values: OrganizationValues) =>
      api.post<Organization>("/api/v1/admin/organizations", { body: values }),
  ...
  const remove = useMutationToast({
    mutationFn: (organization: Organization) =>
      api.delete(`/api/v1/admin/organizations/${organization.id}`),
    ...
    <Card>
      <CardHeader>
        <CardTitle className="flex items-center gap-2">
          <Building2 className="size-4" /> {t("Organizations")}
        </CardTitle>
      </CardHeader>
      <CardContent className="space-y-4 text-sm">
        <p className="text-muted-foreground">
          {t(
            "A grouping of projects for these admin pages. Membership and permissions stay per project.",
          )}
        </p>
        ...
        >
          ...
          <Field
            ...
            error={form.formState.errors.slug?.message}
          >
            <Input id="org-slug" className="w-48" {...form.register("slug")} />
          </Field>
          <Button type="submit" disabled={create.isPending}>
            <Plus className="size-4" /> {t("Add organization")}
          </Button>
        </form>
        {form.formState.errors.root && (
      ...
      <ConfirmDialog
        ...
        description={
          removing
            ? `${removing.name} is removed; its ${removing.project_count} project(s) stay and lose the grouping.`
            : ""
        }
