...
export function TrafficTable({
  ...
  const columns: ColumnDef<TrafficRow, unknown>[] = [
      ...
      cell: ({ getValue }) => (
        <>
          <span className="sm:hidden">
            {formatTimeShort(getValue<string>())}
          </span>
    ...
    ...(showSource
      ? [
          { header: t("Source"), accessorKey: "data_source_name" } as ColumnDef<
            TrafficRow,
            unknown
          >,
        ]
    ...
    ...(showIdentity
      ? [
          ...
          } as ColumnDef<TrafficRow, unknown>,
        ]
      : []),
    {
      header: t("Device"),
      ...
      cell: ({ row }) =>
        row.original.device_name ?? (
          <span
            ...
            title={row.original.external_id ?? ""}
          >
            {showIdentity ? t("not linked") : (row.original.external_id ?? "")}
          </span>
        ),
    },
    {
      ...
      cell: ({ row }) => (
        <span>
          {channelLabel(row.original.acquisition_channel)}
          <span className="block text-xs text-muted-foreground">
            {methodLabel(row.original.ingestion_method, t)}
          </span>
        </span>
      ),
    },
    { header: t("Type"), accessorKey: "event_type" },
    {
      header: t("Details"),
      id: "details",
      accessorFn: (r) => detailsText(r, t),
      cell: ({ row }) => <Details row={row.original} />,
    },
    {
      header: t("Status"),
      ...
      cell: ({ row }) => (
        <span className="inline-flex items-center gap-1">
          <StatusBadge value={row.original.processing_status} />
...
function detailsText(r: TrafficRow, t: T): string {
  ...
  if (r.acquisition_channel === "lorawan") {
    ...
    if (r.best_rssi != null)
      parts.push(
        `${r.best_rssi.toFixed(0)} dBm${r.best_snr != null ? ` / ${r.best_snr.toFixed(1)} dB` : ""}`,
      );
    if (r.gateway_count)
    ...
    parts.push(s.status === "ok" ? t("ok") : s.status_text);
    if (r.frame_bytes != null)
      parts.push(t("{{count}} bytes", { count: r.frame_bytes }));
    if (s.cep_km != null && (s.status === "ok" || s.status === "mt_too_large"))
      parts.push(t("estimate ±{{km}} km", { km: s.cep_km }));
    if (s.missed_since_last)
      parts.push(
        t("{{count}} sessions missed", { count: s.missed_since_last }),
      );
    if (s.duplicate_of != null)
      parts.push(t("redelivery of #{{id}}", { id: s.duplicate_of }));
    if (r.delivered_at)
      parts.push(t("delivered {{when}}", { when: formatAgo(r.delivered_at) }));
  } else {
    if (r.frame_bytes != null)
      parts.push(t("{{count}} bytes", { count: r.frame_bytes }));
    if (r.delivered_at)
      parts.push(
        r.acquisition_channel === "iridium"
          ? t("delivered {{when}}", { when: formatAgo(r.delivered_at) })
          : r.acquisition_channel === "log_file"
            ? t("uploaded {{when}}", { when: formatAgo(r.delivered_at) })
            : t("synced {{when}}", { when: formatAgo(r.delivered_at) }),
      );
    if (r.f_port != null) parts.push(t("port {{port}}", { port: r.f_port }));
  }
  return parts.join(" · ");
...
function Details({ row }: { row: TrafficRow }) {
  ...
  return (
    <span
      ...
      title={row.delivered_at ? formatTime(row.delivered_at) : undefined}
    >
      {detailsText(row, t)}
    </span>
  );
