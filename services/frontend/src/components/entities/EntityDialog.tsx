...
const schema = z.object({
  name: z.string().min(1, t("Give the entity a name")).max(200),
  entity_type_id: z.string().min(1, t("Choose a type")),
  icon_key: z.string(),
...
export function EntityDialog({
  ...
  const save = useMutationToast({
    mutationFn: (values: Values) => {
      ...
      return entity
        ? api.patch<Entity>(
            `/api/v1/projects/${projectId}/entities/${entity.id}`,
            { body },
          )
        : api.post<Entity>(`/api/v1/projects/${projectId}/entities`, { body });
    },
    invalidate: [
      queryKeys.entities(projectId),
      queryKeys.currentState(projectId),
    ],
    success: entity ? t("Entity updated") : t("Entity created"),
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent>
        <DialogHeader>
          <DialogTitle>
            {entity ? t("Edit entity") : t("New entity")}
          </DialogTitle>
        </DialogHeader>
        <form
          className="space-y-4"
        ...
        >
          ...
          <Field
            ...
            hint={t(
              "The kind of thing, then the species or model; the icon follows the choice",
            )}
            error={form.formState.errors.entity_type_id?.message}
          >
            <EntityTypeSelect
              id="entity_type_id"
              projectId={projectId}
              value={form.watch("entity_type_id")}
              onChange={(v) =>
                form.setValue("entity_type_id", v, { shouldValidate: true })
              }
            />
          </Field>
          <Field
            ...
            htmlFor="entity_icon"
            hint={t(
              "Only when this one entity should look different from its type",
            )}
          >
            <div className="flex items-center gap-2">
              <IconPicker
                id="entity_icon"
                value={form.watch("icon_key")}
                onChange={(v) => form.setValue("icon_key", v)}
                className="flex-1"
              />
              {form.watch("icon_key") && (
                <Button
                  ...
                  variant="ghost"
                  size="sm"
                  onClick={() => form.setValue("icon_key", "")}
                >
                  {t("Use the type's icon")}
                </Button>
              )}
            </div>
          </Field>
          <Field label={t("Status")} htmlFor="status">
            <Select
              ...
              onValueChange={(v) =>
                form.setValue("status", v as Values["status"])
              }
            >
          ...
          <div className="grid grid-cols-2 gap-3">
            <Field
              label={t("Latitude")}
              htmlFor="latitude"
              hint={t("Static location, for infrastructure")}
            >
              <Input
                id="latitude"
                inputMode="decimal"
