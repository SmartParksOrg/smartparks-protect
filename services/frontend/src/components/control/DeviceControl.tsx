...
function ParameterFields({ schema, values, onChange }: { schema: Record<string, unknown>; values: Record<string, unknown>; onChange: (next: Record<string, unknown>) => void }) {
  ...
  if (entries.length === 0) return <p className="text-sm text-muted-foreground">This action takes no parameters.</p>;
  return (
        ...
        {d && (
          <div className="space-y-4 text-sm">
            ...
            <div className="grid grid-cols-2 gap-2">
              <div><span className="text-muted-foreground">Payload</span><div className="font-mono text-xs">port {d.command.f_port ?? "?"}: {d.command.payload_hex ?? "not encoded"}</div></div>
              <div><span className="text-muted-foreground">Platform reference</span><div className="font-mono text-xs">{d.command.provider_ref ?? "none"}</div></div>
              <div><span className="text-muted-foreground">Expires</span><div>{formatTime(d.command.expires_at)}</div></div>
              {d.command.trace_id && projectId && <div><span className="text-muted-foreground">Trace</span><div><Link className="underline" to={`/projects/${projectId}/network/traces?trace=${d.command.trace_id}`}>view processing trace</Link></div></div>}
              {Object.keys(d.command.parameters).length > 0 && <div className="col-span-2"><span className="text-muted-foreground">Parameters</span><JsonView value={d.command.parameters} /></div>}
              {Object.keys(d.command.result).length > 0 && <div className="col-span-2"><span className="text-muted-foreground">Device response</span><JsonView value={d.command.result} /></div>}
            </div>
            <div>
    ...
    <>
      <Card>
        <CardHeader className="flex flex-row items-center justify-between space-y-0">
          <CardTitle className="flex items-center gap-2"><Send className="size-4" /> Control</CardTitle>
          <DropdownMenu>
            <DropdownMenuTrigger asChild><Button size="sm" disabled={available.length === 0}>Actions <ChevronDown className="size-4" /></Button></DropdownMenuTrigger>
        ...
        <CardContent className="space-y-3 text-sm">
          {actions.isSuccess && available.length === 0 && <div className="text-muted-foreground">This device type declares no control actions.</div>}
          {commands.data && commands.data.length === 0 && <div className="text-muted-foreground">No commands yet.</div>}
          <ul className="divide-y">
            {commands.data?.map((c) => (
              <li key={c.id} className="flex cursor-pointer flex-wrap items-center gap-2 py-1.5 hover:bg-accent/40" onClick={() => setSelected(c.id)}>
                ...
                <span className="text-xs text-muted-foreground">{formatAgo(c.created_at)} by {c.actor.kind as string}</span>
          ...
          {queue.data?.supported && (
            <div className="flex flex-wrap items-center gap-2 border-t pt-2">
              ...
              <span>{queue.data.items.length} queued on the platform for {queue.data.external_id}</span>
              {queue.data.items.map((q) => <span key={q.id ?? q.data_hex} className="font-mono text-xs">port {q.f_port}: {q.data_hex}{q.is_pending ? " (pending)" : ""}</span>)}
              {canFlush && queue.data.items.length > 0 && <Button variant="ghost" size="sm" className="ml-auto" onClick={() => setFlushing(true)}><Trash2 className="size-4" /> Flush</Button>}
            </div>
          )}
        </CardContent>
      </Card>
        ...
        <DialogContent>
          ...
          {chosen && (
            <div className="space-y-3">
              ...
              {!chosen.confirms && <p className="text-xs text-muted-foreground">The device sends no answer for this action; the lifecycle ends at the last stage the network reports.</p>}
            </div>
          )}
          <DialogFooter><Button variant="outline" onClick={() => setChosen(null)}>Cancel</Button><Button disabled={send.isPending || (needsConfirmation && !confirmed)} onClick={() => chosen && send.mutate(chosen)}>{send.isPending ? "Sending…" : "Send"}</Button></DialogFooter>
        </DialogContent>
      </Dialog>
      <ConfirmDialog open={flushing} onOpenChange={setFlushing} title="Flush the platform's downlink queue?" description="Every queued downlink is dropped; pending commands will expire." confirmLabel="Flush" pending={flush.isPending} onConfirm={() => flush.mutate()} />
