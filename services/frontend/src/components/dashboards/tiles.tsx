...
export function SavedViewTile({
  ...
  const range = (
    (params.range?.[0] ?? "7d") in RANGE_PRESETS
      ? (params.range?.[0] ?? "7d")
      : "7d"
  ) as RangePreset;
  const bucket = params.bucket?.[0] ?? "auto";
  const aggregates = (params.agg ?? []).filter((a): a is Aggregate =>
    (AGGREGATES as readonly string[]).includes(a),
  );
  const chart = (
    (CHART_TYPES as readonly string[]).includes(params.chart?.[0] ?? "")
      ? params.chart?.[0]
      : "line"
  ) as ChartType;
  ...
  const query = useMemo(() => {
    ...
    for (const a of aggregates.length ? aggregates : ["mean"])
      q.append("agg", a);
    q.set("layout", "series");
    return q.toString();
  }, [metricsWanted, entityIds, window, bucket, aggregates]);
  const series = useQuery({
    queryKey: queryKeys.analyticsSeries(projectId, { q: query, tile: view.id }),
    queryFn: () =>
      api.get<SeriesResponse>(
        `/api/v1/projects/${projectId}/analytics/series?${query}`,
      ),
    enabled: metricsWanted.length > 0,
  ...
  if (metricsWanted.length === 0)
    return (
      <p className="text-sm text-muted-foreground">
        {t("This saved view has no metrics.")}
      </p>
    );
  if (series.isError)
  ...
  return (
    <div className="flex h-full flex-col">
      <SeriesChart
        ...
        type={chart}
        aggregate={aggregates[0] ?? "mean"}
        timezone={timezone}
        labels={labels}
        unit={
          metricsWanted.length === 1
            ? metrics.data?.find((m) => m.key === metricsWanted[0])?.unit
            : null
        }
      ...
      <div className="mt-1 text-right text-xs text-muted-foreground">
        <Link
          className="underline"
          to={`/projects/${projectId}/analyze/explorer?view=${view.id}`}
        >
...
export function MapTile({ projectId }: { projectId: string }) {
  ...
  const features = current.data?.features as unknown as
    CurrentFeature[] | undefined;
  useEffect(() => {
    const map = mapRef.current;
    if (!map || !ready) return;
    ensureEntityLayers(map);
    return bindEntityClicks(
      map,
      () => undefined,
      (lngLat, clusterId) => {
...
export function AlertsTile({
  ...
  const alerts = useQuery({
    ...
    queryFn: () =>
      api.get<PageType<Alert>>(`/api/v1/projects/${projectId}/alerts`, {
        query: { status: "open", limit },
      }),
    refetchInterval: 30_000,
  });
  if (alerts.isError)
    return <p className="text-sm text-destructive">{alerts.error.message}</p>;
  return (
    <ul className="divide-y text-sm">
      {alerts.data?.items.length === 0 && (
        <li className="py-1 text-muted-foreground">{t("No open alerts.")}</li>
      )}
      {alerts.data?.items.map((a) => (
        <li key={a.id} className="flex flex-wrap items-center gap-2 py-1">
          <StatusBadge value={a.severity} />
          <Link
            className="min-w-0 flex-1 truncate underline-offset-2 hover:underline"
            to={`/projects/${projectId}/alerts?event=${a.event_id}`}
          >
            {a.title}
          </Link>
...
export function EventsTile({
  ...
  const events = useQuery({
    queryKey: queryKeys.events(projectId, { limit, tile: true }),
    queryFn: () =>
  ...
  return (
    <ul className="divide-y text-sm">
      {events.data?.items.length === 0 && (
        <li className="py-1 text-muted-foreground">{t("No events yet.")}</li>
      )}
      {events.data?.items.map((e) => (
        <li key={e.id} className="flex flex-wrap items-center gap-2 py-1">
          ...
          <Link
            className="min-w-0 flex-1 truncate underline-offset-2 hover:underline"
            to={`/projects/${projectId}/rules/events?event=${e.id}`}
          >
            {e.title}
          </Link>
          <span
...
export function EntityStatusTile({ projectId }: { projectId: string }) {
  ...
  const features =
    (current.data?.features as unknown as CurrentFeature[] | undefined) ?? [];
  const byStatus = new Map<string, number>();
  let alerting = 0;
  for (const f of features) {
    byStatus.set(
      f.properties.status,
      (byStatus.get(f.properties.status) ?? 0) + 1,
    );
    if ((f.properties.active_alert_count ?? 0) > 0) alerting += 1;
  }
  return (
    <div className="grid grid-cols-2 gap-2 text-sm">
      <div className="rounded-md border p-2">
        ...
        <div className="text-2xl">{current.data?.total ?? "…"}</div>
      </div>
      <div className="rounded-md border p-2">
        <div className="text-xs text-muted-foreground">
          {t("With open alerts")}
        </div>
        <div className="text-2xl">{alerting}</div>
      </div>
