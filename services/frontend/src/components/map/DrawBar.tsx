...
import { MapPin, Minus, Pentagon, X } from "lucide-react";
import { useForm } from "react-hook-form";
import { z } from "zod";

import { Callout } from "@/components/common/Callout";
import { Field } from "@/components/common/FormField";
import type { DrawKind } from "@/components/map/draw";
...
/**
 * The bar of the draw and measure modes (decisions D139 and D141): what to draw, the running
 * length or area of what is drawn, and the way out: Save as feature, or Done for a measurement,
 * which is not kept unless turned into a feature.
 */
  ...
  return (
    <div className="rounded-md border bg-card p-2 text-sm shadow-md">
      ...
      <div className="mt-2 text-xs text-muted-foreground">
        {geometry === null
          ? kind === "point"
            ? t("Click the map to place the point.")
            : t(
                "Click the map to add vertices, click the last one again or press Enter to finish, Escape to cancel.",
              )
          : t(
              "Drag a vertex to move it, a midpoint to add one; Escape clears.",
            )}
      </div>
      {(m.length_m != null || m.area_m2 != null) && (
          ...
          {m.length_m != null && (
            <span>
              <span className="text-muted-foreground">
                {m.area_m2 != null ? t("Perimeter") : t("Length")}{" "}
              </span>
