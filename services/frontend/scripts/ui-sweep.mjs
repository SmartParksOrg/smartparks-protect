...
let current = "";
const fill = (p) => p.replace(":projectId", project.id).replace(":deviceId", devices[0]?.id ?? "").replace(":entityId", entities[0]?.id ?? "").replace(":sourceId", sources[0]?.id ?? "");
for (const p of paths) {
  ...
  if (full.includes("/entities/") && !entities[0]) continue;
