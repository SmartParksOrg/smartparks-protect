...
Gates: a button, a page or a menu entry that needs a permission asks `usePermissions(projectId).can("<key>")` (decision D188), never a role name; the role names appear only where a role is chosen. Pages behind a key sit under `RequireProjectPermission` in `App.tsx`; the navigation model carries `permission` per item and section.
