...
Server admin, System health shows the pipeline per area (architecture 26.2): ingestion, decoding, rules and automation, integrations, device control, exports, backup and recovery. Each area has a status and indicators with a link to where you drill down: Needs attention for rejected messages and unknown devices, the Trace explorer of a project for failures by error code, the automation and integration pages for failed deliveries, Backup and recovery for the copies. Workers, stream lag, dead letters and data sources are below the areas; the analysis worker is one of them, and a silent one only stalls analyses. The rules service raises system alerts for stale workers, dead letters, consumer lag and backup problems; they reach you by email or Telegram through a server-level automation. Every signed-in account also carries a small health dot in the top right corner, fed by `GET /system/status`: it summarises the same state in one colour, slow to leave green, and links server admins here (decision D181, ADR 0028).
