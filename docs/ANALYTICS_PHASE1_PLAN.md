...
- Deployed on the dev server on 2026-09-14 at Tim's word (the playbook with `-e git_version=feature/analytics-movement-grazing`, last at 1036078): the analysis service runs, migration 0029 applied, both modules in the catalogue, a movement run over the two cows of the Smart Parks project and a grazing run over two test zones drawn around the meadow of "Rode geus koe 2081" (features "Analysis test zone West" and "East", marked as management units; both kept runs named "Test: …") complete in about 3 seconds each. Seen and fixed the same afternoon: the chart legend on one scrolling row above the plot, the map fitted to the polygons rather than to a track with a far outlier, the speed histogram on fixed bins so animals share one axis, the period and metric cells by their labels, no rank without a pressure, no overlap for zones that only touch, the rest strip's dates horizontal, the runs list inside a phone screen, the track as a line only on the result map; on Tim's first look the runs moved above the result as a row of chips, so the result takes the whole width; the grazing map became a use-intensity choropleth (every residence cell inside the areas with its animal-hours, carried in `summary.intensity` as a compact grid the page draws), with the tracks and the hotspot outlines as toggles that start off, at Tim's remark that tracks say little about grazing; then, at his next look: every run shows its settings (subjects, areas, period, comparison and every option) in a folded block, "Adjust and run again" loads them into the form, the grid cell takes whole metres, and the result map offers the fixes as points and the live map's heatmap as layers beside the tracks; the result map carries the live map's strips (base map, 3D terrain, full screen top right; zoom, north and fit bottom right). Tim's model of runs, built the same evening: the form is the only place settings are set and every Run is a new run (the rerun endpoint and the "Run again" and "Adjust and run again" buttons are gone; opening a run from the table loads its settings into the form); a run is unsaved until named with "Save…" and expires after `ANALYSIS_RETENTION_DAYS` (7 by default, a change from the 30 days of D197); a run is private to the person who ran it until "Shared with the project" (migration 0030, `analysis_runs.shared`, the list and reads filter on it, project admins see what they can open); one runs table with status, subjects, period, who, when, saved and shared. Tim's next look reshaped the page: no form on the page; the list of analyses is the page, "New analysis" opens a dialog with the form, Run returns to the list where the new run shows its progress and a toast says when it is ready, an opened run sits under "All analyses", and "Edit and run again" opens the same dialog with the run's settings, with "Run as new" or "Run and replace" (the new run takes the old one's name and sharing and the old one goes); the subjects of a result take the brand chart palette everywhere. Still to do: the benchmark section and the drill of section 17.
