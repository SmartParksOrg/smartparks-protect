...
**Table.** The virtualized table of the loaded rows, one row per moment, that scrolls a year of a collar as one list. The time is the first column and the rows come newest first; a click on a header sorts the loaded rows by that column, a second click the other way round, and an empty cell sorts last either way. Under every header is a filter: a piece of text for a text column, and for a number column `> 3.5`, `<= 2`, `3.5-4` or a plain number; a line above the table says how many of the loaded rows match and clears them. The columns are the device type, the data source, the kinds of record the moment holds and the position's kind (on by default, Tim, 2026-09-20), the position's figures, every metric and state field with data in the selection (the `via` field shows the route a record came by, `lorawan`, `webble` or `flash_log`, for a position as well as for a status message, since only status messages store it on the state and the column read as empty on every position row; Tim, 2026-09-20), and the identifiers a reader adds from the column picker: the device and entity ids, the source event, its ingestion time and the trace. The picker is kept per user. A click on a row pins its moment in every view and shows the record plainly: the time, who it belongs to, the position and the values of that moment, with the source event one link away.

**Chart.** One graph with a thin line per metric and per selected animal or collar over a shared time axis, so two collars compare on one canvas. The line follows the selection's perspective: a selected collar is one line whatever animals it tracked, a selected animal one line whatever collars tracked it, and a record of a selected collar that carries no animal stays on the collar's line. The metrics on the chart are the loaded columns with a picker (the first four numeric ones by default). The axes follow the units: the first metric's unit takes the left axis, every other unit the right one, so a temperature sits next to a voltage without a setting. Chips above the graph toggle lines, a crosshair follows the pointer with one card listing every line's value at that moment, and every moment carries a small point (a line above a thousand points shows them under the pointer only). The kind switches between line, scatter (one metric against another, per owner), bar, histogram (of the first metric) and state timeline. Above 50,000 records the chart draws from the aggregate series read (below) with the finest bucket that fits and says so; the drawer keeps loading the rows.
