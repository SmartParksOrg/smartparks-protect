...
- **Cards** per subject: distance, daily distance, median speed, stationary share, the MCP, KDE and corrected KDE 95 % areas, the movement strategy, the number of fixes, and the comparison period's figure beside each.
- **Map**, with the live map's controls (base map, 3D terrain, full screen, zoom, north, fit): the subjects' tracks over the main period (with the fixes as points and a heatmap as further chips) and, toggled by chips, the MCP hull, the KDE isopleths, the corrected KDE isopleths, the hotspot cells and the cluster hulls, each in the subject's colour; a click on a polygon names it with its area and its share of time or fixes.
- **Charts**: daily distance, the speed histogram (fixed bins from under 0.01 to over 10 m/s, so every animal shares one axis), activity by hour of the day, the rose of turning angles, the net squared displacement with the fitted strategy curve dotted over it, and distance by day, twilight and night.
...
- **Warnings** above the results say what the fixes allow: missing fixes against the sampling interval, gaps, irregular sampling, impossible speeds left out, duplicates collapsed, poor GNSS quality, a collar change inside the period, too few fixes for a home range; and, for the phase 2 methods, a range the period does not show (the fixes are still spreading at the longest lags drawn) and a period too short for a strategy.
...
- The **corrected KDE** (decision D244) is our own approximation of an autocorrelation-aware range, named as such and never called AKDE. A semivariogram of the fixes is drawn over lags up to a quarter of the period (pairs at a ladder of index lags, binned by the time they span, so a year costs a few dozen passes and irregular sampling is harmless); an Ornstein-Uhlenbeck curve is fitted to it, giving the range's variance (the plateau) and the autocorrelation time (how long the position takes to decorrelate); the effective sample size follows (the fixes divided by one plus twice their summed autocorrelation), and the bandwidth is the reference rule with that count and that variance in place of the fix count and the sample's spread. Few independent looks at a range call for a wide kernel, which supplies what the fixes did not reach; with many, it is the plain KDE again. A range counts as shown only when the plateau lies inside the lags drawn, which means an autocorrelation time of at most an eighth of the period: stricter than the design's half, deliberately, because over a month a random walk and a range that takes a week to cross fit the same rising curve. Otherwise the run says so and draws nothing. The autocorrelation time and the effective fixes stand in the table so a reader sees why the corrected range is larger.
- The **movement strategy** is read off the net squared displacement per day (the median of each day's fixes), after Bunnefeld and others (2011): four curves are fitted by least squares — resident (an asymptote), migratory (out and back, one pace each way), dispersal (one step to a new plateau) and nomadic (a line through the origin) — and the lowest corrected Akaike information wins. The margin to the runner-up is reported, and under two units the class reads "unclear". A departure has to lie at least a week inside the period, or a step at the start is a plateau and a step at the end a line. A period under sixty days gets no class. The figures people read come with the class: the departure and return days for a migrant and the distance between its ranges, the departure day and distance for a disperser, the typical distance from the start for a resident, the drift per day for a nomad.
...
- The corrected KDE is an approximation, not the AKDE of the reference implementation; it needs the period to show the whole range and says so when it does not.
- The strategy is the best of four idealised curves; a migrant that lingers, or a year that holds two moves, fits none of them well, and the margin says how well the winner fits against the rest, not against the animal.
