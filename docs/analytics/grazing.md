...
"Use against vegetation" is one table: the areas by use per hectare with their index, its change and the level beside it, the hardest used first, so an area grazed hard while greening down comes first. One chart draws the index per area by week, the comparison period faint. The area cards carry the index with its change, and the PDF report both the table and the chart. The result map has a **Vegetation (NDVI)** layer, off until the chip switches it on: a mosaic on the same grid as the use intensity, so the two can be read against each other cell for cell, and a click on a cell gives its index. The five colours divide the cells evenly by rank, a fifth each, and the legend names the index each colour starts at. A landscape is not spread evenly over its range: a run over Horsterwold read 0.40 to 0.90, but nine cells in ten sat above 0.86, so colours drawn straight from lowest to highest would put almost everything in one green. Ranking is what makes the poorer ground inside an area visible; the legend and a click on a cell give the real values. The cells come from the same run: the areas and the cells go up to the provider in one job, so the areas keep their exact mean and the mosaic costs no extra job. A large selection is drawn with coarser cells rather than refused (at most 600 cells; the cell size starts at the method's cell size and doubles until they fit). A cell with no cloud-free week in the period is left out rather than drawn as bare ground.
