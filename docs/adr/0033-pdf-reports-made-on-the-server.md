...
The server makes the PDF. A Jinja template with print CSS is rendered by WeasyPrint (page size and margins, a running title and footer, "Page n of m", tables that break across pages with their header repeated), the charts are drawn by matplotlib in the brand palette as SVG, and the map is a picture: the result polygons drawn in Web Mercator over the standard OpenStreetMap tiles stitched together (a few dozen small images at most, fetched a few at a time with a proper user agent, credited as OpenStreetMap asks), or over MapTiler's static image when the server's key allows static maps, or over a plain background with a scale bar and a north arrow when neither arrives. The live map's OpenFreeMap tiles are vector tiles the browser draws; a server cannot use them for a picture without a whole renderer, which is why the report reads raster tiles. A wide table with a handful of rows is turned on its side so it fits A4 portrait.
