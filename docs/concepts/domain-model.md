...
A device moves to another project from a moment you choose: its first data, the moment it joined its current project, a date, or now. From that moment the device belongs to the new project: the old assignments are cut there, a new one opens, and the records from that moment on, positions, measurements, contacts, state history, events with their alerts, log files and commands, get the new project in the background, the way an assignment repair does. What lies before the moment stays with the old project, whose members keep seeing it; when nothing lies before, the device leaves that project for good. "From now" is a handover: the old project keeps everything until now and sees no new data.

An entity comes along when its whole history moves: every device that ever tracked it is in the move and started tracking it at or after the moment. It keeps its name, its history, its current state and its device assignments, and leaves its group (or joins a group of the new project you choose). An entity that keeps history in the old project, because another device tracked it or this device tracked it before the moment, stays there; the moving device's assignment to it ends at the moment and the device arrives without an entity, which the dialog says before you confirm.

An entity moves whole from its own page or from a selection on the Entities page: its history and its devices come along over the spans they tracked it, so a device that later tracked another animal of the old project keeps that part there (its project assignment is split around the span). A name the new project already has blocks that entity.

Every move shows a preview first, per device and per entity: from when, what comes along, what stays and why, what is skipped. The move runs the same plan. It needs a server admin, or a project admin of every project involved. Moving back does not undo a move: the history follows again.
...
Two more kinds exist since phase 30 (ADR 0036). A device that does not move can be given a place by hand on its page: its location source becomes `static` and nothing it sends moves it afterwards. A sighting by such a placed device writes a position of type `proximity` for the device it heard, at the reader, with a stated radius — which for an animal wearing only a Bluetooth tag is the only position there will ever be. An estimate never displaces a newer fix a device made itself, but a device that has never fixed at all takes it whatever its location setting says. The map panels name every kind that is not the device's own fix, so an estimate is never read as one. Every such kind is named once, in `ESTIMATE_RECORD_TYPES`, and readers test against that set rather than against a single name: a reader that knows only `network` treats the other kinds as fixes, which is how a tag came to look as though it had fixed itself at the reader that heard it. See [Bluetooth contacts](../devices/bluetooth-contacts.md).
