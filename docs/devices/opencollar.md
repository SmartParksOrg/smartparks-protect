...
| --- | --- | --- | --- |
| 2 | GNSS position (u-blox) | Position with altitude, accuracy, satellites, fix type and PDOP; measurements `gnss_fix`, `gnss_time_to_fix`, `gnss_satellites`, `gnss_accuracy`, `gnss_pdop`; speed and course when active tracking is on (`ublox_active_tracking`): on the position row and, as every driver's speed is, as the measurements `speed` (m/s, shown as km/h) and `heading` (degrees from north, written only while moving); the course is read little-endian as the firmware writes it, where the public decoder reads it big-endian | Fix timestamp from the receiver |
...
| 19, 20 | External switch change and status | `switch_active`, `switch_count`; `switch_activated` and `switch_deactivated` events; on a device whose entity is a Trap, a `trap_triggered` reading and a `TRAP_CLOSED` or `TRAP_OPENED` event on a change (phase 32, [Fences and traps](fences-and-traps.md)) | Network receive time |
