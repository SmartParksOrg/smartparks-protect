...
On the **live map** a fence line is drawn in its sections' colours, under Features as its own type. Its panel shows the essentials and nothing more (Tim, 2026-09-19): the level and since when, the line as one bar with each section in its colour and a tick where each monitor stands, one sentence naming what is wrong and where ("2 of 4 sections down, Middle to South corner"), and how many monitors with the newest reading's age. The rest is on the **fence line page**, opened from the panel, the Features list (the bolt icon) or a monitor's entity page: the bar large with the monitors named, a table of the sections, every monitor with its reading and its voltage chart over a day, a week or a month, and the status history (the `FENCE_STATUS` events, newest first).

Project admins set a fence line up on the same page, under **Setup**: the name, the line itself (corrected on a map by dragging its vertices rather than drawn again; the monitors' places along the line and the sections follow at once), the thresholds, and the monitors. The monitors are placed **on a map** (Tim, 2026-09-19): the line is drawn in its sections' colours, the monitors on it are markers that drag along it, and a free Fence monitor from the list beside the map is dropped anywhere on the line. Where the pointer lets go is moved onto the line, and that point becomes the monitor's device's fixed place — over any place the device had, since a drop says the hardware moved. Nobody types coordinates, and a monitor is on the line rather than beside it. A monitor without a device cannot be placed, because the place is the device's; a new monitor is made beside the map with its name and its FenceEdge and then dragged on. A new line starts from the Features page like any other feature, and its page opens for the setup once it is made. The Features list shows the line's level.
