...
Project admins set a fence line up on the same page, under **Setup**: the name, the line itself (corrected on a map by dragging its vertices rather than drawn again; the monitors' places along the line and the sections follow at once), the thresholds, and the monitors on it — an existing Fence monitor entity put on the line, one taken off, or a new one made there with its name, its FenceEdge and the place the post stands. A new line starts from the Features page with **New fence line**. The Features list shows the line's level.
