...
A device sends a measurement nobody defined: the metric registers itself on arrival (its values are stored and charted at once) and waits in Needs attention, New metrics, with the number of devices sending it and the last value; give it a label, a unit and a category there. Until then the Data Explorer marks it "not defined yet".

Records exist but the project does not see them (a raw log or Bluetooth download older than the device's assignment): the device page shows "N records before <date>" with "Extend the assignment back to the first data", which attributes them; the same for the entity assignment afterwards. The rewrite runs in the background: the page shows how many records are done with a bar, holds the device's other assignment changes until it is complete, and the job is listed under `GET /devices/{id}/attribution-jobs` with its trace.

Records inside an assignment that still show the collar instead of the animal (in Explore, a second line named after the device; in the record card, no entity): they were decoded before the assignment existed, on a version that did not yet re-attribute on creation. "Recompute attribution" on the device page's Data tab (`POST /devices/{id}/reattribute`, project admins) gives every record of the device its project and entity again from the assignments as they stand, over the device's whole data by default, as the same background job with the same bar.

A shared network application (one KPN application server for every collar of a subscriber, for example) posts every device it holds, so Needs attention fills with unknown identities in one go. Tick the rows you run here (the header box selects every row of the search), then "Create devices": one device type, optionally a project with the start of its assignments (each identity's first sighting by default, so the retained events from before get the project once decoded; or now; or a date) and an entity type, in which case every device gets an entity with the same name so it appears on the map at once. Names come from the platform where it sends one (ThingPark's device name), else the external id; a name already taken gets the external id appended. "Ignore" on the selection hides the rest. Both actions report what was skipped and why. The retained events are decoded in the background: the toast's "Show progress" leads to the progress card at the top of the page, one row per device with a link to it, and the device's own page says "data coming in" until its walk is done.
