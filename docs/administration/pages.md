...
| Grazing (Analyze, when the module is on) | The form (herd, areas from the project's zones and geofences, period, comparison with the period before, the seasons or another herd, weighting), the runs and the selected run: cards per area, the map with the areas coloured by relative pressure and the hotspots inside them, the daily animal-hours and use-per-hectare charts, the use-and-rest calendar, the tables, the warnings, the limitations | The same keys as Movement; "Analyse grazing" on a group (Groups page, entities list filter) and "Grazing in this area" on a zone (live map panel, Features page) open the form with it chosen |
